...
try {
  const sourceRoot = path.join(staging, "source"); fs.mkdirSync(sourceRoot);
  const archivePath = path.join(staging, "source.tar"); const archiveFd = fs.openSync(archivePath, "wx", 0o600);
  ...
  for (const name of ["opsworkbench-agent.service", "opsworkbench-agent-updater.service", "opsworkbench-agent-updater.path"]) fs.copyFileSync(path.join(cleanControlCenter, "deploy", "systemd", name), path.join(unitDir, name));
