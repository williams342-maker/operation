...
const EXAMPLES: Array<{ id: string; kind: string; icon: any; prompt: string; outcome: string }> = [
  { id: "hvac", kind: "Business website", icon: Wrench, prompt: "A modern website for a family-run HVAC company that wants more service calls.", outcome: "A one-page template preview and an editable brief. Booking is not connected." },
  { id: "waitlist", kind: "Launch page", icon: Rocket, prompt: "A launch page for a productivity app with an email waitlist.", outcome: "A single-page template preview. Email collection is Upcoming." },
  { id: "portfolio", kind: "Creator portfolio", icon: User, prompt: "A portfolio site for a freelance photographer showcasing recent shoots.", outcome: "A portfolio brief and one-page template. Galleries are Upcoming." },
  { id: "store", kind: "Online storefront", icon: Store, prompt: "An online store to sell handmade ceramic mugs with a simple catalog.", outcome: "A storefront concept preview. Catalog and checkout are Upcoming." },
...
const CAPABILITIES = [
  ...
  "Storefront concepts", "Campaign concepts", "Event concepts",
];
...
const CONTROLS: Array<{ title: string; body: string; icon: any }> = [
  { title: "Review before publishing", body: "Approve your preview explicitly. Production publishing is currently disabled.", icon: ShieldCheck },
  { title: "Transparent activity", body: "A timeline shows exactly what Foundry is doing at each step.", icon: Eye },
  { title: "Editable content and structure", body: "Edit the brief before approval and review the resulting preview.", icon: PencilRuler },
  { title: "Revision checks", body: "Stale edits are blocked. Per-project undo is Upcoming.", icon: History },
  { title: "Project history", body: "Your project, latest preview and decision history are saved.", icon: History },
    ...
    <header className="sticky top-0 z-30 border-b border-border bg-background/85 backdrop-blur">
      <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-3 px-4 py-3 sm:px-6">
        ...
        <div className="flex items-center gap-2">
          ...
          {authed ? (
            <>
              <button type="button" onClick={onViewProjects} className="inline-flex min-h-11 items-center rounded-md border border-border px-3 py-2 text-sm text-text hover:bg-panel sm:inline-flex">My Projects</button>
            ...
            <>
              <button type="button" onClick={onSignIn} className="inline-flex min-h-11 items-center rounded-md border border-border px-3 py-2 text-sm text-text hover:bg-panel sm:inline-flex">Sign In</button>
              <button type="button" onClick={() => { trackFoundry("foundry_hero_cta_clicked", { location: "topbar" }); onStart(""); }} className="inline-flex min-h-11 items-center gap-1.5 rounded-md bg-primary px-3 py-2 text-sm font-semibold text-primaryForeground hover:bg-primary/90">Start Building</button>
            </>
          )}
        </div>
    ...
    <div className="min-h-screen bg-background text-text">
      ...
      <main>
        ...
        <Section id="foundry-preview" title="See it take shape" description="You watch the project come together as it is built — with editable content and human approval before anything is published.">
          <div className="grid items-center gap-8 lg:grid-cols-[1.3fr_1fr]">
            ...
            <ul className="space-y-3">
              {["Desktop and mobile previews", "Clear progress stages", "Editable project brief", "Reviewable improvements", "Human approval before publishing", "Project history"].map((item) => (
        ...
        <Section id="foundry-control" title="Built for control" description="Foundry assists — it never takes over. You stay in charge of what gets built and published.">
          ...
          <p className="mt-6 rounded-xl border border-border bg-panel p-4 text-sm text-muted">
            <span className="font-semibold text-text">Upcoming:</span> transparent, budget-aware AI credits and additional live AI providers. Today Foundry builds your first preview with its built-in generator, and production publishing is disabled.
          </p>
        </Section>
      ...
      <footer className="border-t border-border py-8">
        <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-3 px-4 text-xs text-muted sm:px-6">
          ...
          <span>Preview approval is explicit. Publishing is disabled.</span>
        </div>
      </footer>
    </div>
  );
