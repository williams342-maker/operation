...
export async function replaceOwner(input: { ownerEmail: string; ownerName: string; password: string }) { const { data } = await api.post("/auth/owner-replacement", input); sessionExpiryReported = false; localStorage.setItem("cc.csrf", data.csrfToken); return data; }
...
export async function googleSignInStart() { return (await api.get("/auth/google/start")).data as { enabled: boolean; clientId?: string; nonce?: string }; }
export async function googleSignIn(credential: string) { const { data } = await api.post("/auth/google", { credential }); sessionExpiryReported = false; localStorage.setItem("cc.csrf", data.csrfToken); return data; }
