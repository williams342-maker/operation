...
export async function replaceOwner(input: { organizationSlug: string; ownerEmail: string; ownerName: string; password: string }) { const { data } = await api.post("/auth/owner-replacement", input); sessionExpiryReported = false; localStorage.setItem("cc.csrf", data.csrfToken); return data; }
