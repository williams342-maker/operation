...
export function AiWorkforcePage({ toast }: { toast: (message: string) => void }) {
  ...
  return <div className="space-y-5">
    ...
    <div><h2 className="mb-3 text-lg font-semibold">Run history</h2>{runs.isLoading ? <Skeleton /> : !(runs.data || []).length ? <Card><p className="text-sm text-muted">No workforce runs have been queued.</p></Card> : <div className="space-y-3">{runs.data.map((run: any) => <Card key={run._id}><div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="font-medium">{roles.find((item) => item.id === run.roleId)?.name || run.roleId}</h3><p className="text-xs text-muted">{resourceLabels[run.resourceType]} / {run.provider} / {run.model}</p><p className="mt-1 text-xs text-muted">Created {new Date(run.createdAt).toLocaleString()}</p>{run.resultSummary && <p className="mt-3 max-w-3xl text-sm">{run.resultSummary}</p>}{run.failureCategory && <p role="alert" className="mt-3 text-sm text-danger">Run failed: {String(run.failureCategory).replace(/_/g, " ")}</p>}</div><div className="flex items-center gap-2"><Badge tone={run.state === "succeeded" ? "success" : run.state === "failed" ? "danger" : "warning"}>{run.state}</Badge>{run.state === "queued" && <Button disabled={cancel.isPending} onClick={() => cancel.mutate(run._id)}>Cancel</Button>}</div></div></Card>)}</div>}</div>
  </div>;
}
