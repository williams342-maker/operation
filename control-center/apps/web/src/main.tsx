...
function AiWebsiteBuilderPage() {
  ...
  if (workflow) {
    ...
    const active = stageIndex[workflow.stage] ?? 0; const selectedBrand = workflow.brandDirections?.find((item: any) => item.id === workflow.selectedBrandId) || workflow.brandDirections?.[0];
    return <div className="mx-auto max-w-7xl text-slate-950"><div className="grid gap-5 xl:grid-cols-[220px_minmax(0,1fr)_240px]">
      ...
      <main className="min-w-0 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
        ...
        {workflow.stage === "preview_ready" && <><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-wide text-emerald-600">Validated responsive preview</p><h2 className="mt-2 text-2xl font-bold">Desktop and mobile ready</h2></div><span className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-semibold text-emerald-700">{workflow.validation.checks} checks passed</span></div><div className="mt-5 flex gap-2" aria-label="Preview viewport"><GhostButton aria-pressed={previewViewport === "desktop"} onClick={() => setPreviewViewport("desktop")}>Desktop</GhostButton><GhostButton aria-pressed={previewViewport === "mobile"} onClick={() => setPreviewViewport("mobile")}>Mobile</GhostButton></div><div className={`mx-auto mt-4 overflow-hidden rounded-2xl border border-slate-300 shadow-xl transition-all ${previewViewport === "mobile" ? "max-w-sm" : "max-w-none"}`} style={{ backgroundColor: selectedBrand?.colors?.[2], color: selectedBrand?.colors?.[0] }}><header className="flex items-center justify-between border-b border-black/10 px-6 py-4"><strong>{workflow.brief.business.name}</strong>{previewViewport === "desktop" ? <nav className="flex gap-4 text-xs">{workflow.architecture.navigation.map((item: any) => <span key={item.route}>{item.title}</span>)}</nav> : <span aria-label="Mobile menu">☰</span>}</header>{workflow.sections.map((section: any) => <section key={section.id} className={`px-6 py-10 text-center ${section.type === "hero" && previewViewport === "desktop" ? "py-20" : "border-t border-black/10"}`}><h3 className={section.type === "hero" ? previewViewport === "desktop" ? "text-5xl font-bold" : "text-3xl font-bold" : "text-2xl font-bold"}>{section.heading}</h3><p className="mx-auto mt-4 max-w-2xl leading-7 opacity-75">{section.body}</p>{section.cta && <button className="mt-6 rounded-lg px-5 py-3 text-sm font-semibold text-white" style={{ backgroundColor: selectedBrand?.colors?.[1] }}>{section.cta}</button>}</section>)}</div><div className="mt-5 flex flex-wrap gap-3"><Button className="bg-emerald-600 text-white" disabled={action.isPending} onClick={() => approve("approve-preview")}>Approve preview for staging</Button><GhostButton onClick={() => setEditing({ ...workflow.sections[0] })}>Edit hero section</GhostButton><GhostButton disabled={regenerate.isPending} onClick={() => regenerate.mutate(workflow.sections[0].id)}>Regenerate hero</GhostButton></div></>}
