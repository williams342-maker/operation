...
function Sparkline({ points }: { points: string }) {
  // Decorative trend; stroke inherits the parent's semantic text color (currentColor) — no hardcoded hue.
  return <svg viewBox="0 0 200 36" role="img" aria-label="Recent trend" className="mt-3 h-9 w-full"><polyline points={points} fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" /></svg>;
}

type MetricTone = "primary" | "success" | "warning" | "muted";
function DashboardMetric({ icon: Icon, label, value, note, tone = "primary", points }: { icon: any; label: string; value: React.ReactNode; note: string; tone?: MetricTone; points?: string }) {
  const badge = tone === "success" ? "bg-success/15 text-success" : tone === "warning" ? "bg-warning/15 text-warning" : tone === "muted" ? "bg-border text-muted" : "bg-primary/15 text-primary";
  const trend = tone === "success" ? "text-success" : tone === "warning" ? "text-warning" : tone === "muted" ? "text-muted" : "text-primary";
  return <section className="rounded-lg border border-border bg-panel p-5 shadow-sm">
    <div className="flex items-start gap-3"><span className={`rounded-lg p-2.5 ${badge}`}><Icon className="h-5 w-5" /></span><div><div className="text-sm text-muted">{label}</div><div className="mt-1 text-3xl font-semibold text-text">{value}</div><div className="mt-1 text-xs text-muted">{note}</div></div></div>
    {points && <div className={trend}><Sparkline points={points} /></div>}
  </section>;
}
  ...
  return (
    <div className="-m-5 min-h-[calc(100vh-5rem)] p-5 text-text lg:p-8">
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <DashboardMetric icon={Boxes} label="Projects" value={d?.projectCount ?? 0} note="Active projects" tone="primary" points={dashboardSeries.projects} />
        <DashboardMetric icon={Server} label="Servers" value={d?.serverCount ?? 0} note={`${d?.onlineServers ?? 0} online`} tone="success" points={dashboardSeries.servers} />
        <DashboardMetric icon={Activity} label="Recent activity" value={recent.length} note="Latest recorded events" tone="muted" points={dashboardSeries.activity} />
        <DashboardMetric icon={ShieldCheck} label="System Health" value={(d?.serverCount ?? 0) === (d?.onlineServers ?? 0) ? "Healthy" : "Review"} note={`${d?.onlineServers ?? 0} of ${d?.serverCount ?? 0} servers online`} tone={(d?.serverCount ?? 0) === (d?.onlineServers ?? 0) ? "success" : "warning"} />
      </div>
      <div className="mt-4 grid gap-4 xl:grid-cols-[1.4fr_1fr]">
        <section className="rounded-lg border border-border bg-panel p-5 shadow-sm"><div className="flex items-center justify-between"><h3 className="text-lg font-semibold text-text">Activity overview</h3><span className="inline-flex items-center gap-2 rounded-md border border-border px-3 py-2 text-xs text-muted"><CalendarDays className="h-4 w-4" /> Last 30 days</span></div><svg viewBox="0 0 700 220" className="mt-5 w-full" role="img" aria-label="Activity trend"><g className="stroke-border" strokeWidth="1">{[25,70,115,160,205].map((y) => <line key={y} x1="35" y1={y} x2="680" y2={y} />)}</g><polyline className="stroke-primary" points="35,175 90,160 145,185 200,145 255,165 310,125 365,95 420,115 475,165 530,125 585,80 630,105 680,38" fill="none" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" /></svg></section>
        <section className="rounded-lg border border-border bg-panel p-5 shadow-sm"><div className="flex items-center justify-between"><h3 className="text-lg font-semibold text-text">Recent activity</h3><button onClick={() => onNavigate("audit")} className="rounded text-sm font-medium text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-background">View all</button></div><div className="mt-4 divide-y divide-border">{recent.length ? recent.map((event: any) => <div key={event._id || `${event.action}-${event.createdAt}`} className="flex items-center gap-3 py-3"><span aria-hidden="true" className={`grid h-7 w-7 place-items-center rounded-full text-sm ${event.result === "success" ? "bg-success/15 text-success" : "bg-warning/15 text-warning"}`}>{event.result === "success" ? "✓" : "!"}</span><span className="sr-only">{event.result === "success" ? "Success" : "Attention"}: </span><div className="min-w-0 flex-1"><div className="truncate text-sm font-medium text-text">{event.action}</div><div className="text-xs text-muted">{event.targetType || "OpsWorkbench"}</div></div><div className="text-xs text-muted">{fmt(event.createdAt)}</div></div>) : <p className="py-10 text-center text-sm text-muted">No recent activity</p>}</div></section>
      </div>
      <div className="mt-4 grid gap-4 xl:grid-cols-[1fr_1.5fr]">
        <section className="rounded-lg border border-border bg-panel p-5 shadow-sm"><h3 className="text-lg font-semibold text-text">System health</h3><p className="mt-1 text-xs text-muted">Per-server CPU and memory are shown on server details.</p>{[["CPU usage","Not collected here",Activity],["Memory usage","Not collected here",Database],["Network",d?.onlineServers === d?.serverCount ? "Healthy" : "Review",LineChart]].map(([label,value,Icon]: any) => <div key={label} className="mt-4 flex items-center justify-between gap-3 text-sm"><span className="inline-flex items-center gap-2 text-muted"><Icon className="h-4 w-4" />{label}</span><span className="text-text">{value}</span></div>)}</section>
        <section className="rounded-lg border border-border bg-panel p-5 shadow-sm"><h3 className="text-base font-semibold text-text">Website Builder</h3><p className="mt-2 max-w-md text-sm text-muted">Guided static-site builder with staged approval and validation. A separate tool, run outside the operations cockpit.</p><div className="mt-4"><GhostButton onClick={() => onNavigate("ai-builder")}>Open builder</GhostButton></div></section>
      </div>
    </div>
  );
}
