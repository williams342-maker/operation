...
function AiWebsiteBuilderPage() {
  ...
  const create = useMutation({ mutationFn: () => api.post("/website-builder/workflows", { ...(selection ? { websiteType: selection.type } : {}), objective }).then((r) => r.data), onSuccess: (data) => { setWorkflow(data.workflow); setQuestion(data.question); } });
  ...
  return <div className="mx-auto max-w-6xl text-slate-950">
    <section className="overflow-hidden rounded-3xl bg-gradient-to-br from-blue-50 via-white to-emerald-50 p-6 shadow-sm sm:p-10">
      ...
      <p className="mt-3 max-w-2xl text-slate-600">Describe the outcome in your own words. OpsWorkbench turns it into a reviewable discovery workflow and asks one useful question at a time. Nothing is generated or deployed yet.</p>
      <label className="mt-7 block text-sm font-semibold text-slate-800" htmlFor="website-objective">What do you want to build or improve?</label>
      <textarea id="website-objective" value={objective} onChange={(event) => setObjective(event.target.value)} rows={5} minLength={12} maxLength={4000} placeholder="Example: Build a warm, trustworthy online store for handmade ceramics, with simple product discovery and an email signup before our October launch." className="mt-2 w-full rounded-2xl border border-slate-300 bg-white p-4 text-base shadow-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100" />
      <div className="mt-2 flex justify-between text-xs text-slate-500"><span>Plain-language planning uses no provider credits.</span><span>{objective.length}/4000</span></div>
      <h3 className="mt-8 font-semibold">Optional: choose a starting format</h3><p className="mt-1 text-sm text-slate-500">Leave this unselected and OpsWorkbench will recommend a format from your objective.</p>
      ...
      <div className="mt-7 flex flex-wrap items-center justify-between gap-4 rounded-2xl border border-slate-200 bg-white p-4"><div><div className="font-medium">{selection?.title || "Format will be recommended from your objective"}</div><div className="mt-1 text-sm text-slate-500">The next step is discovery. AI credit estimates appear before any paid work.</div></div><Button disabled={objective.trim().length < 12 || create.isPending} onClick={() => create.mutate()} className="bg-gradient-to-r from-blue-600 to-emerald-500 px-5 text-white">{create.isPending ? "Starting..." : "Plan this website"}</Button></div>
      <ErrorText error={create.error} />
      {!!history.data?.length && <div className="mt-6 rounded-2xl border border-slate-200 bg-white p-4"><h3 className="font-semibold">Continue a saved website</h3><div className="mt-3 grid gap-2 sm:grid-cols-2">{history.data.map((item: any) => <button key={item.id} className="rounded-xl border p-3 text-left text-sm hover:border-blue-400" onClick={() => api.get(`/website-builder/workflows/${item.id}`).then((r) => { setWorkflow(r.data.workflow); setQuestion(r.data.question); })}><span className="font-medium capitalize">{item.websiteType.replace(/_/g, " ")}</span><span className="ml-2 text-xs text-slate-500">{item.stage.replace(/_/g, " ")}</span>{item.objective && <span className="mt-1 block truncate text-xs text-slate-500">{item.objective}</span>}</button>)}</div></div>}
