...
describe("foundry suggestions", () => {
  ...
  it("offers a revision-bound hero rewrite without claiming undo", () => {
    const hero = buildSuggestions(base).find((s) => s.id === "regen-hero:1");
    expect(hero?.action).toEqual({ kind: "regenerate", sectionId: "hero" });
    expect(hero?.reversible).toBe(false);
  });

  it("recommends a contact page only when the project has none", () => {
    expect(buildSuggestions(base).some((s) => s.id === "add-contact:1")).toBe(true);
  ...
  it("binds recommendations to artifact revision rather than decision-only updates", () => {
    const before = buildSuggestions(base).find(s => s.id.startsWith("a11y-contrast:"));
    const after = buildSuggestions({ ...base, version: 2 }).find(s => s.id.startsWith("a11y-contrast:"));
