...
// Uses the semantic `danger` token (no hardcoded color) and keeps geometry/focus-ring identical to GhostButton.
export function DangerButton(props: ButtonHTMLAttributes<HTMLButtonElement>) { return <button {...props} className={`inline-flex min-h-[2.5rem] items-center justify-center gap-2 rounded-md border border-danger/50 px-3 py-2 text-sm text-danger transition-colors hover:bg-danger/10 disabled:opacity-50 ${focusRing} ${props.className || ""}`} />; }
