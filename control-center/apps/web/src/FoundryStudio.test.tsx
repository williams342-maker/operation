...
const STAGE_FOR_PATH: Record<string, string> = {
  "prepare-preview": "preview_ready", "approve-brief": "architecture_review", "approve-architecture": "brand_review", "select-brand": "content_review",
...
describe("Foundry studio", () => {
  ...
  it("auto-advances reversible stages to a ready preview, then stops at the human approval boundary", async () => {
    ...
    // auto-crossed.
    expect(screen.getByRole("button", { name: /approve preview/i })).toBeEnabled();
  ...
  it("shows the original prompt as user-provided and never presents an invented business name", async () => {
    ...
    expect(screen.getAllByText(/template suggestion/i).length).toBeGreaterThan(0);
  });
  ...
  it("names an unnamed project inline and refreshes the preview without creating another workflow", async () => {
    ...
    expect(await screen.findByRole("heading", { name: "Cedar & Sage", level: 1 })).toBeInTheDocument();
  });
});

it("persists typed work across unmount and reload", async () => {
  const view=render(<FoundryStudio route={{kind:"new"}} navigate={vi.fn()} />);
  await userEvent.type(screen.getByLabelText(/describe what you want to build/i), "Build a preserved draft");
  view.unmount(); render(<FoundryStudio route={{kind:"new"}} navigate={vi.fn()} />);
  expect(screen.getByLabelText(/describe what you want to build/i)).toHaveValue("Build a preserved draft");
});

it("clears the old preview immediately when another project is denied", async () => {
  mocks.get.mockResolvedValueOnce({workflow:wf("preview_ready",{artifact,validation})}).mockRejectedValueOnce(new Error("Workflow not found"));
  const view=render(<FoundryStudio route={{kind:"project",workflowId:"w1"}} navigate={vi.fn()} />);
  await screen.findByTitle("Generated website preview");
  view.rerender(<FoundryStudio route={{kind:"project",workflowId:"w2"}} navigate={vi.fn()} />);
  expect(screen.queryByTitle("Generated website preview")).toBeNull();
  await screen.findByText("Workflow not found");
  expect(screen.queryByTitle("Generated website preview")).toBeNull();
});

it("does not record human approvals while preparing a template", async () => {
  mocks.get.mockResolvedValue({workflow:wf("brief_review")});
  render(<FoundryStudio route={{kind:"project",workflowId:"w1"}} navigate={vi.fn()} />);
  await screen.findByTitle("Generated website preview");
  expect(mocks.advance.mock.calls.map(call=>call[1])).toEqual(["prepare-preview"]);
});

it("keeps a dirty brief bound to the revision originally edited after refresh", async () => {
  mocks.get.mockResolvedValueOnce({workflow:wf("preview_ready",{artifact,validation})})
    .mockResolvedValueOnce({workflow:wf("preview_ready",{version:2,artifact,validation,brief:{...brief,business:{name:"Other tab",description:PROMPT}}})});
  mocks.updateBrief.mockRejectedValue(new Error("Project changed; reload before saving"));
  render(<FoundryStudio route={{kind:"project",workflowId:"w1"}} navigate={vi.fn()} />);
  await screen.findByTitle("Generated website preview");
  await userEvent.type(screen.getByRole("textbox",{name:"Business or project name"}),"My unsaved name");
  await userEvent.click(screen.getByRole("button",{name:"Refresh"}));
  await screen.findByRole("heading",{name:"Other tab",level:1});
  ...
  expect(mocks.updateBrief.mock.calls[0][2]).toBe(1);
  expect(screen.getByRole("textbox",{name:"Business or project name"})).toHaveValue("My unsaved name");
});

it("does not redirect or clear a newer draft when creation completes after navigation", async () => {
  let finish: (value:any)=>void=()=>{};
  mocks.create.mockReturnValue(new Promise(resolve=>{finish=resolve;}));
  const navigate=vi.fn();const view=render(<FoundryStudio route={{kind:"new"}} navigate={navigate}/>);
  await userEvent.type(screen.getByRole("textbox",{name:"Describe what you want to build"}),"First draft");
  await userEvent.click(screen.getByRole("button",{name:"Start Building"}));
  view.unmount(); sessionStorage.setItem("foundry.draftPrompt","Newer draft");
