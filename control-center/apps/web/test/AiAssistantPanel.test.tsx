...
test("assistant panel exposes operational intelligence follow-ups", () => { const html = render(); assert.match(html, /current health evidence/); assert.match(html, /similar to a previous incident/); assert.match(html, /Analyze evidence/); });
test("assistant panel exposes a bounded workforce role selector", () => { const html = render(); assert.match(html, /AI workforce role/); assert.match(html, /operations_analyst/); assert.match(html, /roleId/); });
