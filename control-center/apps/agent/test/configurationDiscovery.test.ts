...
test("configuration discovery returns names and metadata but never environment values", () => {
  ...
  try {
    fs.writeFileSync(path.join(root, ".env.example"), "SERVICE_API_KEY=example-placeholder\nPUBLIC_ORIGIN=https://example.invalid\n", "utf8");
    fs.writeFileSync(path.join(root, ".env"), "SERVICE_API_KEY=do-not-return-this-value\n", "utf8");
    ...
    assert.ok(result.settings.some((setting) => setting.name === "SERVICE_API_KEY" && setting.configured));
