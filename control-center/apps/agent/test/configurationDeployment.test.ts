...
test("agent rejects a redirect to a private target and rolls back safely", async () => { resetReplayStateForTests(); const item = fixture(); let calls = 0; const result = await executeConfigurationDeployment(item.payload, item.key, "redirect-private", [...deploymentCapabilities], "0.1.0", { ...safeNetwork, compose: async () => ({ code: 0 }), httpRequest: async () => ++calls === 1 ? { statusCode: 302, location: "http://169.254.169.254/latest/meta-data" } : { statusCode: 200 }, healthRetryWindowMs: 1, healthRetryIntervalMs: 0 }); assert.equal(result.phase, "rolled_back"); assert.equal(calls, 2); assert.equal(fs.readFileSync(item.env, "utf8"), "PUBLIC=value\n"); });
