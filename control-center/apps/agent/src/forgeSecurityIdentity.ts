...
export const forgeSecurityIdentitySchema = z.object({
  ...
  trustedRootSha256: z.string().regex(/^[a-f0-9]{64}$/), hostname: z.string().min(1),
...
export type SecurityPathPolicy = {
  directory: string; identityPath: string; trustedRootPath: string; expectedUid: number; expectedGid: number;
...
export function forgeSecurityIdentityStatement(identity: Omit<ForgeSecurityIdentity, "ownerSignature">): Buffer {
  return Buffer.from([identity.schemaVersion, identity.orgId, identity.serverId, identity.ownerPublicKey,
    identity.trustedRootSha256, identity.hostname, identity.machineIdSha256, identity.validFrom,
...
export function loadForgeSecurityMaterial(policy: SecurityPathPolicy = {
...
}): { identity: ForgeSecurityIdentity; trustedRoot: unknown } {
  ...
  if (path.dirname(path.resolve(policy.identityPath)) !== directory || path.dirname(path.resolve(policy.trustedRootPath)) !== directory) {
  ...
  if (policy.mountInfoPath) assertNotSubstitutedMount([directory, policy.identityPath, policy.trustedRootPath], policy.mountInfoPath);
