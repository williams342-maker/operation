...
test("implementation plan is preview-only and carries rollback guidance", () => {
  ...
  assert.match(plan.rollback, /undo is Upcoming/);
});
...
test("preview links remain in the isolated artifact and all navigation targets exist", () => {
  const architecture = buildArchitecture(brief); const artifact = buildStaticSiteArtifact(brief, architecture, buildBrandDirections(brief)[0], buildSiteContent(brief, architecture));
  assert.match(artifact.html, /default-src 'none'/);
  assert.doesNotMatch(artifact.html, /href="(?:https?:|\/)/);
  for (const match of artifact.html.matchAll(/href="#([^"]+)"/g)) assert.ok(artifact.html.includes('id="' + match[1] + '"'));
  assert.equal(buildValidation([]).checks, 3);
});
