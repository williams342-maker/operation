...
test("database-backed Phase 1B API and fake-agent verification", { skip: !enabled, timeout: 120_000 }, async () => {
  ...
  try {
    ...
    // NO ORGANISATION ID IN THE RESPONSE. Security review required that the control plane cannot supply
    // either half of the identity a host matches an owner-signed Forge document against, so this asserts
    // the absence rather than the shape — a shape check would pass the moment somebody added it back.
    const identity = heartbeat.body as { orgId?: string; serverId?: string };
    assert.equal("orgId" in identity, false, "the control plane must not be able to tell an agent which organisation it belongs to");
    assert.equal(identity.serverId, credentials.serverId, "the long-standing server id is unchanged, and the agent no longer reads it for identity");
