...
import { agentSigningKey, deploymentCapabilities, signRequest } from "@control-center/shared";
...
test("database-backed Phase 1B API and fake-agent verification", { skip: !enabled }, async () => {
  ...
  try {
    ...
    await collections.servers.updateOne({ _id: new ObjectId(credentials.serverId), orgId: orgA._id }, { $set: { agentVersion: "0.1.0", agentCapabilities: [...deploymentCapabilities] } });
    const targetProfile = { projectId: project.body.id, environmentId: safeEnvironment.body.id, serverId: credentials.serverId, repositoryRoot: "/srv/phase-1b", environmentFilePath: "/srv/phase-1b/env/.env.staging", composePath: "/srv/phase-1b/docker-compose.yml", composeOverridePaths: ["/srv/phase-1b/app.override.yml"], composeProject: "phase-1b", statelessServices: ["api", "web"], protectedServices: ["mongo"], healthChecks: [{ id: "public-health", url: "https://example.com/healthz", timeoutMs: 5000 }], currentConfigurationDigest: "a".repeat(64) };
    const anonymousTarget = await request("POST", "/configuration/deployment-targets", targetProfile, { "content-type": "application/json" });
    assert.equal(anonymousTarget.status, 401);
    const viewerTarget = await request("POST", "/configuration/deployment-targets", targetProfile, jsonHeaders(viewerA));
    assert.equal(viewerTarget.status, 403);
    const invalidTarget = await request("POST", "/configuration/deployment-targets", { ...targetProfile, credential: "not-allowed" }, jsonHeaders(ownerA));
    assert.equal(invalidTarget.status, 400);
    const createdTarget = await request<{ id: string; revision: number }>("POST", "/configuration/deployment-targets", targetProfile, jsonHeaders(ownerA));
    assert.equal(createdTarget.status, 201);
    assert.equal(createdTarget.body.revision, 1);
    const duplicateTarget = await request("POST", "/configuration/deployment-targets", targetProfile, jsonHeaders(ownerA));
    assert.equal(duplicateTarget.status, 409);
    const viewerTargets = await request<any>("GET", `/configuration/deployment-targets?projectId=${project.body.id}&environmentId=${safeEnvironment.body.id}`, undefined, jsonHeaders(viewerA));
    assert.equal(viewerTargets.status, 200);
    assert.equal(viewerTargets.body.targets.length, 1);
    assert.equal(viewerTargets.body.targets[0].enabled, true);
    assert.equal(Object.prototype.hasOwnProperty.call(viewerTargets.body.targets[0], "repositoryRoot"), false);
    const viewerTargetDetail = await request("GET", `/configuration/deployment-targets/${createdTarget.body.id}`, undefined, jsonHeaders(viewerA));
    assert.equal(viewerTargetDetail.status, 403);
    const ownerTargetDetail = await request<any>("GET", `/configuration/deployment-targets/${createdTarget.body.id}`, undefined, jsonHeaders(ownerA));
    assert.equal(ownerTargetDetail.status, 200);
    assert.equal(ownerTargetDetail.body.target.repositoryRoot, targetProfile.repositoryRoot);
    const editedProfile = { ...targetProfile, statelessServices: ["api"] };
    const editedTarget = await request<{ id: string; revision: number }>("PATCH", `/configuration/deployment-targets/${createdTarget.body.id}`, editedProfile, jsonHeaders(ownerA));
    assert.equal(editedTarget.status, 201);
    assert.equal(editedTarget.body.revision, 2);
    const targetsAfterEdit = await request<any>("GET", `/configuration/deployment-targets?projectId=${project.body.id}&environmentId=${safeEnvironment.body.id}`, undefined, jsonHeaders(ownerA));
    assert.equal(targetsAfterEdit.body.targets.length, 2);
    assert.equal(targetsAfterEdit.body.targets[0].enabled, true);
    assert.equal(targetsAfterEdit.body.targets[1].enabled, false);
    const activeTarget = targetsAfterEdit.body.targets[0];
    const disabledTarget = await request<any>("POST", `/configuration/deployment-targets/${activeTarget._id}/status`, { enabled: false, expectedUpdatedAt: activeTarget.updatedAt }, jsonHeaders(ownerA));
    assert.equal(disabledTarget.status, 200);
    const targetAfterDisable = await collections.configurationTargetProfiles.findOne({ _id: new ObjectId(activeTarget._id), orgId: orgA._id });
    assert.equal(targetAfterDisable?.enabled, false);
    const enabledTarget = await request<any>("POST", `/configuration/deployment-targets/${activeTarget._id}/status`, { enabled: true, expectedUpdatedAt: targetAfterDisable!.updatedAt.toISOString() }, jsonHeaders(ownerA));
    assert.equal(enabledTarget.status, 200);
    assert.equal(await collections.auditEvents.countDocuments({ orgId: orgA._id, action: "configuration.target.create", result: "success" }), 1);
    assert.equal(await collections.auditEvents.countDocuments({ orgId: orgA._id, action: "configuration.target.create", result: "failure" }), 1);
    assert.equal(await collections.auditEvents.countDocuments({ orgId: orgA._id, action: "configuration.target.update", result: "success" }), 1);
    assert.equal(await collections.auditEvents.countDocuments({ orgId: orgA._id, action: "configuration.target.disable", result: "success" }), 1);
    assert.equal(await collections.auditEvents.countDocuments({ orgId: orgA._id, action: "configuration.target.enable", result: "success" }), 1);
    ...
    const plannedDeployment = await request<any>("POST", `/projects/${project.body.id}/deployments`, { requestedRevision: "abc1234", environment: "staging" }, jsonHeaders(ownerA));
    assert.equal(plannedDeployment.status, 201, JSON.stringify(plannedDeployment.body));
    assert.equal(plannedDeployment.body.deployment.status, "planned");
    assert.equal(plannedDeployment.body.deployment.validation.health, "not_run");
    assert.equal(plannedDeployment.body.deployment.validation.readiness, "not_run");
    assert.equal(plannedDeployment.body.deployment.rollbackAvailable, false);
    assert.match(plannedDeployment.body.deployment.planDigest, /^[a-f0-9]{64}$/);
    assert.ok(Date.parse(plannedDeployment.body.deployment.approvalExpiresAt) > Date.now());
    ...
    const exactApproval = { planDigest: plannedDeployment.body.deployment.planDigest, confirm: true };
    assert.equal((await request("POST", `/projects/${project.body.id}/deployments/${plannedDeployment.body.deployment.id}/approve`, { planDigest: "f".repeat(64), confirm: true }, jsonHeaders(administratorA))).status, 404);
    const selfApproval = await request<any>("POST", `/projects/${project.body.id}/deployments/${plannedDeployment.body.deployment.id}/approve`, exactApproval, jsonHeaders(ownerA));
    assert.equal(selfApproval.status, 409);
    assert.match(selfApproval.body.error, /creator cannot approve/i);
    const approvedDeployment = await request<any>("POST", `/projects/${project.body.id}/deployments/${plannedDeployment.body.deployment.id}/approve`, exactApproval, jsonHeaders(administratorA));
    ...
    assert.equal((await request("POST", `/projects/${project.body.id}/deployments`, { requestedRevision: "abc1234", environment: "staging" }, jsonHeaders(overviewViewer))).status, 403);
    ...
    const disposableTemporaryPassword = "temporary-password-long";
    const disposable = await request<{ id: string; mustChangePassword: boolean; oneTimePassword?: string }>("POST", "/org/users", { email: "disposable@example.test", name: "Disposable User", role: "Viewer", password: disposableTemporaryPassword }, jsonHeaders(ownerA));
    assert.equal(disposable.status, 201);
    assert.equal(disposable.body.mustChangePassword, true);
    assert.equal(disposable.body.oneTimePassword, undefined);
    await login("phase-1b-a", "disposable@example.test", disposableTemporaryPassword);
    ...
    const oldPasswordRejected = await request("POST", "/auth/login", { organizationSlug: "phase-1b-a", email: "disposable@example.test", password: disposableTemporaryPassword }, { "content-type": "application/json" });
    ...
    assert.equal(await collections.sessions.countDocuments({ userId: new ObjectId(disposable.body.id) }), 0);
    const sensitiveAuditSnapshot = JSON.stringify(await collections.auditEvents.find({ action: { $in: ["user.create", "user.password.change", "user.password.reset", "user.delete"] } }).toArray());
    ...
    assert.equal(sensitiveAuditSnapshot.includes(disposableTemporaryPassword), false);
