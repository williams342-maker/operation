...
export class DeterministicMockProvider implements AiProvider {
  ...
  async analyze(request: AiProviderRequest): Promise<AiAssistantResponse> {
    const parsed = JSON.parse(request.context) as { scope?: { label?: string }; snapshot?: { scope?: { label?: string }; rejectedSections?: Array<{ key: string }> }; evidence?: AiAssistantResponse["evidence"]; likelyCauses?: AiAssistantResponse["likelyCauses"]; recommendedSteps?: AiAssistantResponse["recommendedSteps"]; confidence?: AiAssistantResponse["confidence"]; timeline?: AiAssistantResponse["timeline"]; relatedIncidents?: AiAssistantResponse["relatedIncidents"] };
    const evidence = parsed.evidence || [];
    return { summary: `Read-only evidence analysis for ${parsed.snapshot?.scope?.label || parsed.scope?.label || "the selected resource"}.`, status: evidence.some((item) => /unhealthy|failed|offline|critical/i.test(item.value)) ? "warning" : "unknown", confidence: parsed.confidence || "low", risk: "low", likelyCauses: parsed.likelyCauses || [], recommendedSteps: parsed.recommendedSteps || [], evidence, alternativePossibilities: parsed.likelyCauses?.length ? ["A less common cause may exist outside the currently collected evidence."] : ["No evidence-backed alternative can be ranked from the available snapshot."], timeline: parsed.timeline || [], relatedIncidents: parsed.relatedIncidents || [], limitations: [`${parsed.snapshot?.rejectedSections?.length || 0} stale, incomplete, or unavailable context sections were excluded.`, "The assistant is read-only and secrets are excluded."], executedActions: [], generatedAt: new Date(0).toISOString() };
  }
}
...
class AnthropicProvider implements AiProvider {
  readonly name = "anthropic"; lastUsage?: { inputTokens?: number; outputTokens?: number };
