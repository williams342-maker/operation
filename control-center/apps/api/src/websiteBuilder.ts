...
export function buildImplementationPlan(architecture: any, sections: SiteSection[]) {
  return { version: 1, routeCount: architecture.pages.length, componentCount: sections.length, files: ["downloadable static HTML preview"], tests: ["responsive layout", "keyboard navigation", "heading hierarchy", "metadata"], estimatedCredits: 0, repositoryMutation: false, deploymentImpact: "Preview only; staging requires separate approval.", rollback: "Revert the source release; saved project content is retained. Per-project undo is Upcoming.", approved: false };
}
...
export function buildValidation(sections: SiteSection[]) {
  ...
  return { passed: findings.length === 0, checks: 3, warnings: findings, reviewedAt: new Date() };
}
...
export function buildStaticSiteArtifact(brief: any, architecture: any, brand: any, sections: SiteSection[]) {
  ...
  const navigation = sections.map(section => `<a href="#${escapeHtml(section.id)}">${escapeHtml(section.type === "hero" ? "Home" : section.type === "cta" ? "Contact" : section.type)}</a>`).join("");
  const content = sections.map((section) => `<section id="${escapeHtml(section.id)}" class="section ${escapeHtml(section.type)}"><div class="inner"><h${section.type === "hero" ? "1" : "2"}>${escapeHtml(section.heading)}</h${section.type === "hero" ? "1" : "2"}><p>${escapeHtml(section.body)}</p>${section.cta ? `<a class="button" href="#cta">${escapeHtml(section.cta)}</a>` : ""}</div></section>`).join("");
  const css = `:root{color-scheme:light;--bg:${background};--text:${text};--accent:${accent}}*{box-sizing:border-box;overflow-wrap:anywhere}a:focus-visible{outline:3px solid currentColor;outline-offset:4px}body{margin:0;background:var(--bg);color:var(--text);font-family:Inter,ui-sans-serif,system-ui,sans-serif;line-height:1.6}header,footer{display:flex;align-items:center;justify-content:space-between;gap:2rem;padding:1rem clamp(1rem,5vw,4rem);border-bottom:1px solid color-mix(in srgb,var(--text) 12%,transparent)}footer{border-top:1px solid color-mix(in srgb,var(--text) 12%,transparent);border-bottom:0}nav{display:flex;flex-wrap:wrap;gap:1rem}a{color:inherit;text-decoration:none}.section{padding:clamp(3rem,8vw,7rem) clamp(1rem,5vw,4rem);text-align:center;border-bottom:1px solid color-mix(in srgb,var(--text) 10%,transparent)}.inner{max-width:64rem;margin:auto}h1{font-size:clamp(2.4rem,7vw,5.5rem);line-height:1.05;margin:0}h2{font-size:clamp(1.8rem,4vw,3rem);line-height:1.15;margin:0}.section p{max-width:48rem;margin:1.25rem auto 0}.button{display:inline-block;margin-top:2rem;padding:.85rem 1.25rem;border-radius:.65rem;background:var(--accent);color:#fff;font-weight:700}@media(max-width:640px){header,footer{flex-wrap:wrap}header nav{display:flex}.section{padding-block:3.5rem}}`;
  const description = escapeHtml(brief.business.description).slice(0, 160);
  const html = `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta http-equiv="Content-Security-Policy" content="default-src 'none'; style-src 'unsafe-inline'; base-uri 'none'; form-action 'none'"><title>${escapeHtml(displayName)}</title><meta name="description" content="${description}"><meta property="og:title" content="${escapeHtml(displayName)}"><meta property="og:description" content="${description}"><style>${css}</style></head><body><header><strong>${escapeHtml(displayName)}</strong><nav aria-label="Primary">${navigation}</nav></header><main>${content}</main><footer><strong>${escapeHtml(displayName)}</strong><p>Preview only. Contact forms, checkout and additional pages are not connected.</p></footer></body></html>`;
