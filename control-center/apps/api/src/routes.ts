...
router.post("/agent/poll", requireSignedAgent, async (req, res, next) => {
  try {
    ...
    // NO ORGANISATION ID HERE, deliberately. It was added so an agent could learn which organisation it
    // belongs to; security review required that the control plane not be able to supply either half of
    // the identity that decides which owner-signed Forge document a host will accept. The agent takes
    // both from that document instead. The server id stays because it long predates this and the agent
    // no longer reads it for identity — but nothing here may grow into that role again.
