...
import { aiAssistantRequestSchema, aiWorkforceRole, aiWorkforceRoles } from "@control-center/shared";
import { audit } from "./audit.js";
import { requirePermission, noStore } from "./auth.js";
import { collections } from "./db.js";
import { aiAssistantConfig, assistantSystemPromptForRole, callProvider, organizationProvider, questionDigest } from "./aiAssistant.js";
...
aiAssistantRouter.post("/ai-assistant/analyze", noStore, requirePermission("ai:use"), async (req, res) => {
  ...
  try {
    ...
    const role = aiWorkforceRole(body.roleId);
    if (!role.allowedScopeTypes.includes(scopeType as "server" | "application")) return res.status(403).json({ error: "AI workforce role is not allowed for this resource type", code: "role_scope_not_allowed" });
    ...
    const result = await callProvider(provider, { system: assistantSystemPromptForRole(body.roleId), context: bounded.serialized, question: String(question.value), maxOutputTokens: config.maxOutputTokens }, config.timeoutMs);
