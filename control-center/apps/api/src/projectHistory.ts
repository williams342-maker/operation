...
export function deploymentHistoryItem(row: ProjectDeploymentDoc, serverName: string | undefined, role: Role): ProjectDeploymentHistoryItem {
  return { id: id(row._id), projectId: id(row.projectId), server: { id: id(row.serverId), name: safeText(serverName, 120) }, environment: safeText(row.environment, 80) || "unknown", requestedRevision: safeRevision(row.requestedRevision) || "unknown", deployedRevision: safeRevision(row.deployedRevision), branch: safeRevision(row.branch), artifactDigest: safeDigest(row.artifactDigest), releaseId: safeText(row.releaseId, 160), taskId: id(row.taskId), ...(hasPermission(role, "audit:view") ? { actor: { id: id(row.actorId) } } : {}), status: row.status, startedAt: iso(row.startedAt), completedAt: iso(row.completedAt), validation: { health: row.validation.health, readiness: row.validation.readiness, checkedAt: iso(row.validation.checkedAt) }, rollbackAvailable: row.rollbackAvailable, evidenceConfidence: row.evidenceConfidence, failureClassification: safeText(row.failureClassification, 80), createdAt: row.createdAt.toISOString() };
}
