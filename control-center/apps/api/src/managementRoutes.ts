...
import { cloudflareOnboardingSchema, deriveWebsiteTarget, enrollmentInstallCommand, enrollmentInstallScript, evaluateStagingBurnIn, projectDeploymentCapabilities, validateConfiguredPath } from "@control-center/shared";
...
import { discoverWebsite, validatePublicHealthCheckUrl, websiteFailureStatus } from "./urlDiscovery.js";
...
managementRouter.post("/projects/:id/deployments", noStore, requirePermission("projects:manage"), async (req, res, next) => { try { if (!ObjectId.isValid(String(req.params.id))) return res.status(404).json({ error: "Project not found" }); const id = new ObjectId(String(req.params.id)); const org = orgId(req); const body = projectDeploymentPlanSchema.parse(req.body); const project = await collections.projects.findOne({ _id: id, orgId: org, ...notArchived }, { projection: { name: 1, primaryServerId: 1, branch: 1 } }); if (!project?._id) return res.status(404).json({ error: "Project not found" }); const server = await collections.servers.findOne({ _id: project.primaryServerId, orgId: org, ...notArchived }, { projection: { name: 1 } }); if (!server?._id) return res.status(400).json({ error: "Project server is unavailable" }); const now = new Date(); const approvalExpiresAt = new Date(now.getTime() + 15 * 60 * 1000); const deploymentId = new ObjectId(); const taskId = new ObjectId(); const creatorId = actorId(req); const requestedRevision = body.requestedRevision.toLowerCase(); const branch = project.branch || "main"; const planDigest = createHash("sha256").update(JSON.stringify({ schemaVersion: "project-deployment-plan-v1", deploymentId: deploymentId.toHexString(), projectId: id.toHexString(), serverId: project.primaryServerId.toHexString(), environment: body.environment, requestedRevision, branch, taskId: taskId.toHexString(), creatorId: creatorId.toHexString(), createdAt: now.toISOString(), approvalExpiresAt: approvalExpiresAt.toISOString() })).digest("hex"); const result = await collections.projectDeployments.insertOne({ _id: deploymentId, orgId: org, projectId: id, serverId: project.primaryServerId, environment: body.environment, requestedRevision, branch, taskId, actorId: creatorId, planDigest, approvalExpiresAt, status: "planned", validation: { health: "not_run", readiness: "not_run" }, rollbackAvailable: false, evidenceConfidence: "reported", auditEventIds: [], createdAt: now, updatedAt: now }); await audit({ orgId: org, actorType: "user", actorId: creatorId, action: "project.deployment.plan", targetType: "project-deployment", targetId: result.insertedId, result: "success", requestId: req.requestId, metadata: { projectId: id.toHexString(), environment: body.environment, status: "planned" } }); const created = await collections.projectDeployments.findOne({ _id: result.insertedId, orgId: org }); if (!created) throw new Error("Deployment plan record was not created"); res.status(201).json({ deployment: deploymentHistoryItem(created, server.name, req.user!.role) }); } catch (error) { next(error); } });
managementRouter.post("/projects/:id/deployments/:deploymentId/approve", noStore, requireRecentAuth, requirePermission("projects:manage"), async (req, res, next) => { try {
  const body = z.object({ planDigest: z.string().regex(/^[a-f0-9]{64}$/), confirm: z.literal(true) }).strict().parse(req.body);
  if (!ObjectId.isValid(String(req.params.id)) || !ObjectId.isValid(String(req.params.deploymentId))) return res.status(404).json({ error: "Pending deployment plan not found" });
  const projectId = new ObjectId(String(req.params.id)); const deploymentId = new ObjectId(String(req.params.deploymentId)); const org = orgId(req); const approverId = actorId(req);
  const plan = await collections.projectDeployments.findOne({ _id: deploymentId, orgId: org, projectId, status: "planned", planDigest: body.planDigest });
  if (!plan?._id) return res.status(404).json({ error: "Pending deployment plan not found or digest changed" });
  if (plan.actorId.equals(approverId)) return res.status(409).json({ error: "Deployment creator cannot approve the same plan" });
  ...
  const now = new Date();
  const approved = await collections.projectDeployments.findOneAndUpdate({ _id: deploymentId, orgId: org, projectId, status: "planned", planDigest: body.planDigest, approvalExpiresAt: { $gt: now }, actorId: { $ne: approverId } }, { $set: { status: "approved", approvedByUserId: approverId, approvedAt: now, updatedAt: now } }, { returnDocument: "after" });
  if (!approved) return res.status(409).json({ error: "Deployment plan is no longer pending approval" });
  const server = await collections.servers.findOne({ _id: approved.serverId, orgId: org }, { projection: { name: 1 } });
  await audit({ orgId: org, actorType: "user", actorId: approverId, action: "project.deployment.approve", targetType: "project-deployment", targetId: deploymentId, result: "success", requestId: req.requestId, metadata: { projectId: projectId.toHexString(), environment: approved.environment, status: "approved" } });
  res.json({ deployment: deploymentHistoryItem(approved, server?.name, req.user!.role) });
} catch (error) { next(error); } });
managementRouter.post("/projects/:id/deployments/:deploymentId/cancel", noStore, requireRecentAuth, requirePermission("projects:manage"), async (req, res, next) => { try {
  z.object({ confirmation: z.literal("CANCEL") }).strict().parse(req.body);
  if (!ObjectId.isValid(String(req.params.id)) || !ObjectId.isValid(String(req.params.deploymentId))) return res.status(404).json({ error: "Cancellable deployment plan not found" });
...
managementRouter.post("/projects/:id/deployments/:deploymentId/preflight", noStore, requireRecentAuth, requirePermission("projects:manage"), async (req, res, next) => { try {
  const body = z.object({ planDigest: z.string().regex(/^[a-f0-9]{64}$/), confirm: z.literal(true) }).strict().parse(req.body);
  if (!ObjectId.isValid(String(req.params.id)) || !ObjectId.isValid(String(req.params.deploymentId))) return res.status(404).json({ error: "Approved deployment plan not found" });
  const projectId = new ObjectId(String(req.params.id)); const deploymentId = new ObjectId(String(req.params.deploymentId)); const org = orgId(req); const checkedByUserId = actorId(req);
  const plan = await collections.projectDeployments.findOne({ _id: deploymentId, orgId: org, projectId, status: "approved", planDigest: body.planDigest });
  if (!plan?._id) return res.status(404).json({ error: "Approved deployment plan not found or digest changed" });
  const [project, server, queuedTask] = await Promise.all([collections.projects.findOne({ _id: projectId, orgId: org, ...notArchived }), collections.servers.findOne({ _id: plan.serverId, orgId: org, ...notArchived }), collections.agentTasks.findOne({ _id: plan.taskId, orgId: org })]);
  const recentHeartbeat = Boolean(server?.lastHeartbeatAt && Date.now() - server.lastHeartbeatAt.getTime() <= 5 * 60 * 1000);
  const checks = [{ name: "project_active", passed: Boolean(project) }, { name: "server_active", passed: Boolean(server) }, { name: "non_production_environment", passed: ["staging", "preview", "testing"].includes(plan.environment) }, { name: "agent_online", passed: server?.status === "online" }, { name: "agent_heartbeat_recent", passed: recentHeartbeat }, { name: "git_revision_preflight_supported", passed: Boolean(server?.agentCapabilities?.includes("gitRevisionPreflight")) }, { name: "execution_not_queued", passed: !queuedTask }];
  const status = checks.every((check) => check.passed) ? "passed" as const : "failed" as const; const now = new Date();
  const updated = await collections.projectDeployments.findOneAndUpdate({ _id: deploymentId, orgId: org, projectId, status: "approved", planDigest: body.planDigest }, { $set: { controlPlanePreflight: { status, checks, checkedAt: now, checkedByUserId }, updatedAt: now } }, { returnDocument: "after" });
  if (!updated) return res.status(409).json({ error: "Deployment plan changed during preflight" });
  await audit({ orgId: org, actorType: "user", actorId: checkedByUserId, action: "project.deployment.preflight", targetType: "project-deployment", targetId: deploymentId, result: status === "passed" ? "success" : "failure", requestId: req.requestId, metadata: { projectId: projectId.toHexString(), status, failedChecks: checks.filter((check) => !check.passed).length } });
  res.status(status === "passed" ? 200 : 409).json({ deployment: deploymentHistoryItem(updated, server?.name, req.user!.role) });
} catch (error) { next(error); } });
managementRouter.post("/projects/:id/deployments/:deploymentId/git-preflight", noStore, requireRecentAuth, requirePermission("projects:manage"), async (req, res, next) => { try {
  const body = z.object({ planDigest: z.string().regex(/^[a-f0-9]{64}$/), confirm: z.literal(true) }).strict().parse(req.body);
  if (!ObjectId.isValid(String(req.params.id)) || !ObjectId.isValid(String(req.params.deploymentId))) return res.status(404).json({ error: "Approved deployment plan not found" });
  const projectId = new ObjectId(String(req.params.id)); const deploymentId = new ObjectId(String(req.params.deploymentId)); const org = orgId(req); const requestedByUserId = actorId(req);
  const plan = await collections.projectDeployments.findOne({ _id: deploymentId, orgId: org, projectId, status: "approved", planDigest: body.planDigest, "controlPlanePreflight.status": "passed", approvalExpiresAt: { $gt: new Date() } });
  if (!plan?._id) return res.status(409).json({ error: "A current approved plan and passed control-plane preflight are required" });
  if (plan.gitPreflight) return res.status(409).json({ error: "Git preflight already exists for this immutable plan" });
...
managementRouter.post("/projects/:id/deployments/:deploymentId/execute", noStore, requireRecentAuth, requirePermission("projects:manage"), async (req, res, next) => { try {
  const body = z.object({ planDigest: z.string().regex(/^[a-f0-9]{64}$/), confirm: z.literal("DEPLOY_WITH_AUTOMATIC_ROLLBACK") }).strict().parse(req.body);
  if (!ObjectId.isValid(String(req.params.id)) || !ObjectId.isValid(String(req.params.deploymentId))) return res.status(404).json({ error: "Executable deployment plan not found" });
  const projectId = new ObjectId(String(req.params.id)); const deploymentId = new ObjectId(String(req.params.deploymentId)); const org = orgId(req); const requestedByUserId = actorId(req); const now = new Date();
  const plan = await collections.projectDeployments.findOne({ _id: deploymentId, orgId: org, projectId, status: "approved", planDigest: body.planDigest, approvalExpiresAt: { $gt: now }, "controlPlanePreflight.status": "passed", "gitPreflight.status": "passed" });
  if (!plan?._id || !plan.gitPreflight?.resolvedRevision || !plan.gitPreflight.headRevision || !plan.gitPreflight.checkedAt || !plan.gitPreflight.checks.every((check) => check.passed)) return res.status(409).json({ error: "A current approved plan with passed control-plane and exact-revision Git preflight is required" });
  if (now.getTime() - plan.gitPreflight.checkedAt.getTime() > 5 * 60 * 1000) return res.status(409).json({ error: "Git preflight is stale; create and approve a new deployment plan" });
  if (!["staging", "preview", "testing"].includes(plan.environment)) return res.status(409).json({ error: "Production deployment execution is unavailable" });
  const environmentKind = plan.environment as "staging" | "preview" | "testing";
  const [project, server, environment] = await Promise.all([collections.projects.findOne({ _id: projectId, orgId: org, ...notArchived }), collections.servers.findOne({ _id: plan.serverId, orgId: org, ...notArchived }), collections.configurationEnvironments.findOne({ orgId: org, projectId, kind: environmentKind, protected: false })]);
  if (!project?.repoPath || !server?._id || !environment?._id || server.status !== "online" || !server.lastHeartbeatAt || now.getTime() - server.lastHeartbeatAt.getTime() > 5 * 60 * 1000) return res.status(409).json({ error: "The active project, online server, and non-production environment are required" });
  const profile = await collections.configurationTargetProfiles.findOne({ orgId: org, projectId, environmentId: environment._id, serverId: server._id, enabled: true });
  if (!profile?._id || profile.repositoryRoot !== project.repoPath) return res.status(409).json({ error: "An enabled reviewed target profile bound to the registered repository is required" });
  const missingCapabilities = projectDeploymentCapabilities.filter((capability) => !(server.agentCapabilities || []).includes(capability));
  if (missingCapabilities.length) return res.status(409).json({ error: "The server agent does not support project deployment with automatic rollback", missingCapabilities });
  const payload = { schemaVersion: "project-deployment-v1" as const, action: "project.deploy.v1" as const, deploymentId: deploymentId.toHexString(), planDigest: body.planDigest, environmentKind: plan.environment as "staging" | "preview" | "testing", protected: false as const, targetProfileId: profile._id.toHexString(), targetProfileRevision: profile.revision, repositoryRoot: profile.repositoryRoot, composePath: profile.composePath, composeOverridePaths: profile.composeOverridePaths || [], composeProject: profile.composeProject, statelessServices: profile.statelessServices, protectedServices: profile.protectedServices, healthChecks: profile.healthChecks, branch: plan.branch || project.branch || "main", expectedCurrentRevision: plan.gitPreflight.headRevision, requestedRevision: plan.gitPreflight.resolvedRevision, automaticRollback: true as const };
  const task = await createTask({ orgId: org, server: server as typeof server & { _id: ObjectId }, projectId, taskId: plan.taskId, type: "project.deploy", payload: { projects: [], httpHealthChecks: [], mongoChecks: [], projectDeployment: payload }, idempotencyKey: `project-deployment-execute:${deploymentId.toHexString()}:${body.planDigest}`, createdByUserId: requestedByUserId, expiresAt: new Date(now.getTime() + 30 * 60 * 1000) });
  const updated = await collections.projectDeployments.findOneAndUpdate({ _id: deploymentId, orgId: org, projectId, status: "approved", planDigest: body.planDigest, approvalExpiresAt: { $gt: now }, taskId: task._id }, { $set: { status: "preparing", startedAt: now, targetProfileId: profile._id, targetProfileRevision: profile.revision, updatedAt: now } }, { returnDocument: "after" });
  if (!updated) { await collections.agentTasks.updateOne({ _id: task._id, orgId: org, state: "queued" }, { $set: { state: "cancelled", cancelledAt: now, completedAt: now, updatedAt: now }, $inc: { version: 1 } }); return res.status(409).json({ error: "Deployment plan changed while execution was queued" }); }
  await audit({ orgId: org, actorType: "user", actorId: requestedByUserId, action: "project.deployment.start", targetType: "project-deployment", targetId: deploymentId, result: "success", requestId: req.requestId, metadata: { projectId: projectId.toHexString(), environment: plan.environment, automaticRollback: true, targetProfileRevision: profile.revision } });
  res.status(202).json({ deployment: deploymentHistoryItem(updated, server.name, req.user!.role) });
} catch (error) { next(error); } });
