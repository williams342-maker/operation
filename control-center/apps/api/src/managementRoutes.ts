...
managementRouter.post("/projects/:id/deployments/:deploymentId/approve", noStore, requireRecentAuth, requirePermission("projects:manage"), async (req, res, next) => { try {
  ...
  const plan = await collections.projectDeployments.findOne({ _id: deploymentId, orgId: org, projectId, status: "planned" });
  if (!plan?._id) return res.status(404).json({ error: "Pending deployment plan not found" });
  if (plan.actorId.equals(approverId)) return res.status(409).json({ error: "Deployment creator cannot approve the same plan" });
  const now = new Date();
  const approved = await collections.projectDeployments.findOneAndUpdate({ _id: deploymentId, orgId: org, projectId, status: "planned", actorId: { $ne: approverId } }, { $set: { status: "approved", approvedByUserId: approverId, approvedAt: now, updatedAt: now } }, { returnDocument: "after" });
