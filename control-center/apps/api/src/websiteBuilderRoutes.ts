...
import { buildArchitecture, buildBrandDirections, buildImplementationPlan, buildProjectBrief, buildSiteContent, buildValidation, regenerateSiteSection } from "./websiteBuilder.js";
...
websiteBuilderRouter.post("/website-builder/workflows/:id/approve-implementation", noStore, requirePermission("ai:use"), async (req, res, next) => {
  try { const workflow = await loadWorkflow(req, res); if (!workflow) return; if (workflow.stage !== "implementation_approval" || !workflow.implementationPlan || !workflow.sections) return res.status(409).json({ error: "Implementation plan is not awaiting approval" }); const validation = buildValidation(workflow.sections as any); const updated = await recordApproval(req, workflow, "implementation_plan", Number((workflow.implementationPlan as any).version) || 1, { stage: "preview_ready", validation, actualCredits: 0 }); if (!updated) return res.status(409).json({ error: "Workflow changed; reload and try again" }); res.json(workflowResponse(updated)); } catch (error) { next(error); }
});
