...
// AI-first entry: one natural-language sentence becomes a ready-to-review brief.
// The provider "understands" the request (deterministic default, live AI later),
// then builds the brief through the same seam. No paid work runs unless a ready,
// budgeted provider is configured — the deterministic path is free.
websiteBuilderRouter.post("/website-builder/workflows/from-prompt", noStore, requirePermission("ai:use"), async (req, res, next) => {
  try {
    const body = z.object({ prompt: z.string().trim().min(3).max(4000), websiteType: websiteTypeSchema.optional() }).strict().parse(req.body);
    const websiteType = websiteTypeSchema.parse(body.websiteType || inferWebsiteType(body.prompt));
