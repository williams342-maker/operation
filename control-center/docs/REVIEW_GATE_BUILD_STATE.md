...
| **BLOCKING (was MAJOR)** | The Mongo store is unverified, and this now **blocks the next candidate**, not just this one. The approved split-authority checklist requires conformance against a real replica set — a skipped Mongo suite is explicitly not sufficient for approval. This machine has no running Docker and no mongod. **Owner decision on footprint:** install MongoDB, add mongodb-memory-server as a devDependency, or run conformance on a host that already has a replica set. |
