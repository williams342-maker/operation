...
| **BLOCKING (open, new)** | **The §2.6 sequence has no actor that can execute it.** Only the audience principal may reserve; only the lease holder may bind, acquire and redeem — so one principal must do all four, and `acquire`/`redeem` happen on the host, so that principal is the executor. But `bind` needs the final payload, and the only channel delivering payloads to an executor is the dispatch that comes *after* bind and sign. **Activating an executor today would refuse every privileged task, permanently.** Not a vulnerability — every executor is DISABLED — but it is why activation cannot be switched on. See `REVIEW_GATE_DISPATCH_GAP.md`. Needs independent confirmation, then an owner decision on where reserve/bind happen. |
