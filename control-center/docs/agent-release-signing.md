...
Publication requires all repository tests, reproducible-build comparison, signature/tamper checks, secret scan, disposable Debian/Ubuntu x86_64 systemd install, idempotent reinstall, reboot/reconnect, failure rollback, and owner review. Artifact hosting, Cloudflare machine-access policy, production signing-key custody, retention, monitoring, and any production/canary execution remain separate owner actions.
