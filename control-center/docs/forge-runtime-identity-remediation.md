...
| 4 | **Owner key.** Preserve the reviewed public-key trust relationship; do not rotate in this sequence *unless the existing private half is unavailable*, in which case treat rotation as a new trust-anchor change requiring review | **open, owner's — and this condition has now fired.** A rotation is authorised and is being prepared under review; see below |
...
### Condition 4: the truncated tail, recovered

An earlier revision of this section recorded condition 4's text as **truncated mid-sentence**, at
*"If the key must be rotated, treat t…"*, and said the tail had never been received. **The tail has
since been recovered** and the condition reads, complete:

> *"Preserve the currently reviewed public-key trust relationship. Do not rotate the owner key as part of
> this merge/provisioning sequence unless the existing private half is unavailable. If the key must be
> rotated, treat that as a new trust-anchor change requiring review."*

**Recorded as recovered, not as though it had been present all along.** It was unavailable here for
several hours and was supplied afterwards as an owner-provided review record. The earlier revision was
honest about the gap, and this revision is honest about how it closed.

The same verdict carries a custody requirement alongside it, which is not superseded by anything here:

> **The owner private key must never be copied to production, to this repository, to CI, to a database,
> to an evidence bundle, or to any automated working material.** A declaration about the key is all any
> tooling needs; the key itself is never an input.

**The condition has fired, and the consequence is procedural rather than a change of posture.** A
rotation is authorised and is being prepared under review, which is exactly what the recovered sentence
requires: a trust-anchor change gets a review, not a configuration edit. Preparation has begun with test
coverage for the pinned key and its digest, on the grounds that a value about to be edited should not be
the one value nothing tests.

**What has not changed.** The pinned public key is unchanged. No replacement key has been generated by
any automated process, and none may be. Forge remains inactive, no material is installed, no ceremony
has been run, and production is untouched.
