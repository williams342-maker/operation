...
The currently enrolled legacy agents require one manual bootstrap because they do not implement `agentUpgrade` or the updater handoff. No command is generated until an owner publishes a real signed compatible release and provisions its public key/catalog through the approved deployment mechanism. At that point the bootstrap procedure must download to a protected temporary file, verify the catalog digest and signature plus artifact SHA-256 and signature, preserve `agent.json` and credential files, install the fixed systemd templates, validate the service, and remove temporary material. It must not pipe into Bash or place credentials in history or process arguments.
