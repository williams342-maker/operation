...
export type ProjectDeploymentHistoryItem = { id: string; projectId: string; server: { id: string; name?: string }; environment: string; requestedRevision: string; deployedRevision?: string; branch?: string; artifactDigest?: string; releaseId?: string; taskId: string; actor?: { id: string }; approval?: { approverId: string; approvedAt: string }; status: ProjectDeploymentState; startedAt?: string; completedAt?: string; validation: ProjectValidationResult; rollbackAvailable: boolean; evidenceConfidence: "verified" | "reported"; failureClassification?: string; createdAt: string };
