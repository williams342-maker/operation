...
async function seeded(state?: string) {
  ...
  await svc.createCandidate({ candidateId: "c1", binding: binding(), actorIdentity: "claude" });
  if (state) {
    ...
    for (const [to, occ] of path) {
      await svc.transition({
        candidateId: "c1", actorIdentity: "claude", occurrenceId: occ,
...
test("CRITICAL-1: a caller cannot claim its own current state", async () => {
  ...
  const result = await svc.submitVerdict({
    candidateId: "c1", actorIdentity: "codex", occurrenceId: "x1", billingClass: "INTERNAL_REVIEW",
    verdict: {
...
test("CRITICAL-1: a caller cannot supply an empty participation ledger", async () => {
  ...
  const result = await svc.submitVerdict({
    candidateId: "c1", actorIdentity: "claude", occurrenceId: "x2", billingClass: "INTERNAL_REVIEW",
    verdict: {
...
test("CRITICAL-1: the ledger is written by the operations, not by the caller", async () => {
  ...
  assert.deepEqual(
    ...
    [["claude", "author"]],
    "createCandidate must have written the author row itself",
  );
});
...
test("CRITICAL-2: an author cannot submit a verdict naming an uninvolved reviewer", async () => {
  ...
  const result = await svc.submitVerdict({
    candidateId: "c1", actorIdentity: "claude", occurrenceId: "x3", billingClass: "INTERNAL_REVIEW",
    verdict: {
      candidateDigest: candidateDigest(binding()), reviewerIdentity: "codex",
      verdict: "GO", findings: [], submittedAt: "2026-09-02T01:00:00.000Z",
    },
...
test("a genuine independent reviewer still succeeds — the fix is not merely refusing everything", async () => {
  const { store, svc } = await seeded("REVIEW_IN_PROGRESS");
  const result = await svc.submitVerdict({
    candidateId: "c1", actorIdentity: "codex", occurrenceId: "x4", billingClass: "INTERNAL_REVIEW",
    verdict: {
      candidateDigest: candidateDigest(binding()), reviewerIdentity: "codex",
...
test("H12: a replayed occurrence id is a no-op, not a second transition", async () => {
  ...
  const first = await svc.transition({
    candidateId: "c1", actorIdentity: "claude", occurrenceId: "dup", billingClass: "INTERNAL_QA_TEST", to: "TESTED",
  });
  assert.equal(first.ok, true);
  const replay = await svc.transition({
    candidateId: "c1", actorIdentity: "claude", occurrenceId: "dup", billingClass: "INTERNAL_QA_TEST", to: "FROZEN",
  });
...
test("H17: two racing transitions from the same read cannot both apply", async () => {
  ...
  const [a, b] = await Promise.all([
    svc.transition({ candidateId: "c1", actorIdentity: "claude", occurrenceId: "r1", billingClass: "INTERNAL_QA_TEST", to: "TESTED" }),
    svc.transition({ candidateId: "c1", actorIdentity: "claude", occurrenceId: "r2", billingClass: "INTERNAL_QA_TEST", to: "TEST_FAILED" }),
  ]);
...
test("review-gate work cannot be charged to the customer", async () => {
  ...
  const result = await svc.transition({
    candidateId: "c1", actorIdentity: "claude", occurrenceId: "b1",
...
test("a verdict must be classed as review work", async () => {
  const { svc } = await seeded("REVIEW_IN_PROGRESS");
  const result = await svc.submitVerdict({
    candidateId: "c1", actorIdentity: "codex", occurrenceId: "b2", billingClass: "INTERNAL_QA_TEST",
    verdict: {
      candidateDigest: candidateDigest(binding()), reviewerIdentity: "codex",
      verdict: "GO", findings: [], submittedAt: "2026-09-02T01:30:00.000Z",
    },
...
test("MODERATE: an expired candidate cannot be advanced", async () => {
  ...
  await svc.createCandidate({ candidateId: "c2", binding: b, actorIdentity: "claude" });
  const result = await svc.transition({
    candidateId: "c2", actorIdentity: "claude", occurrenceId: "e1", billingClass: "INTERNAL_QA_TEST", to: "TESTED",
  });
  assert.equal(result.ok, false);
...
test("MODERATE: malformed input is a closed decision, never an exception", async () => {
  const { svc } = await seeded("REVIEW_IN_PROGRESS");
  const result = await svc.submitVerdict({
    candidateId: "c1", actorIdentity: "codex", occurrenceId: "m1", billingClass: "INTERNAL_REVIEW",
...
test("an author cannot register a candidate attributed to someone else", async () => {
  ...
  const result = await svc.createCandidate({
    candidateId: "c3", binding: binding({ authorIdentity: "someone-else" }), actorIdentity: "claude",
  });
  assert.equal(result.ok, false);
