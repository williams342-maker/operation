...
test("AI response requires explainability fields and empty executed actions", () => { const base = { summary: "Safe", status: "unknown", confidence: "low", risk: "low", likelyCauses: [], recommendedSteps: [], evidence: [], alternativePossibilities: [], timeline: [], relatedIncidents: [], limitations: [], executedActions: [] }; assert.equal(aiAssistantResponseSchema.safeParse(base).success, true); assert.equal(aiAssistantResponseSchema.safeParse({ ...base, executedActions: [{ command: "restart" }] }).success, false); assert.equal(aiAssistantResponseSchema.safeParse({ ...base, alternativePossibilities: undefined }).success, false); });
