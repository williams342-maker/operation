...
from conftest import _DB_NAME_AT_STARTUP


def test_db_name_is_the_one_the_server_was_started_with():
    now = os.environ.get("DB_NAME")
    assert now == _DB_NAME_AT_STARTUP, (
        ...
        % (_DB_NAME_AT_STARTUP, now, _DB_NAME_AT_STARTUP)
    )


def test_mongo_url_is_unchanged_too():
    # Same class of failure, same silence. Cheap to assert while we are here.
    from conftest import _ENV_AT_STARTUP

    started = _ENV_AT_STARTUP.get("MONGO_URL")
    if not started:
    ...
    assert os.environ.get("MONGO_URL") == started, (
        ...
        % (started, os.environ.get("MONGO_URL"))
    )
