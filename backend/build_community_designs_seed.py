"""
Build the community design library seed.

Each seed design = real, hand-crafted SVG + matching DXF (via ezdxf) +
Nano Banana preview PNG. All files land in
/app/frontend/public/seed-designs/{slug}/ so they ship with the
frontend deploy artifact — no R2 round-trips needed for seeded
downloads.

Idempotent: re-running skips designs whose SVG file already exists,
and upserts the Mongo doc by slug.

Run:
    cd /app/backend && python3 build_community_designs_seed.py
"""
import asyncio
import base64
import os
...
def _design(slug, title, desc, category, technique, tags, w, h, svg, dxf_segments, prompt):
    SEED_DESIGNS.append({
        "slug": slug, "title": title, "description": desc,
        "category": category, "technique": technique, "tags": tags,
...
_design(
    ...
    "Mountain Range Silhouette",
    "Clean 3-peak mountain range outline. Sized for an 18×6 inch wall sign — scales cleanly to any aspect. Single closed path, ready for plasma or laser.",
    "Wall Art", "PLASMA", ["mountain", "outdoor", "rustic", "silhouette"],
    18.0, 6.0,
...
_design(
    ...
    "Heart Monogram Blank",
    "Classic split-heart monogram blank — drop two initials in the side panels and a last initial in the center bar. 12×10, ready for laser engraving.",
    "Wedding Gifts", "LASER", ["heart", "monogram", "wedding", "personalize"],
    12.0, 10.0,
...
_design(
    ...
    "Welcome Arrow Sign Blank",
    "Long horizontal welcome sign blank with a chevron arrow on the right edge. 24×6 — ideal for front-door plaques.",
    "Custom Signs", "PLASMA", ["welcome", "arrow", "front door", "porch"],
    24.0, 6.0,
...
_design(
    "pine-tree-trio",
    "Pine Tree Trio",
    "Three classic pine tree silhouettes for cabin/lodge signage. 12×8, can be split into individual cut paths.",
    "Outdoor Art", "PLASMA", ["pine", "tree", "cabin", "forest"],
    12.0, 8.0,
...
_design(
    "vertical-address-blank",
    "Vertical Address Plaque Blank",
    "Tall narrow plaque blank with rounded corners — sized so 4-digit house numbers in 3.5″ tall font centre cleanly. 4×20.",
    "Address Numbers", "PLASMA", ["address", "numbers", "house", "modern"],
    4.0, 20.0,
...
_design(
    ...
    "Classic 8-petal mandala for decorative laser cuts in wood or acrylic. 10×10, clean closed paths.",
    "Home Decor", "LASER", ["mandala", "geometric", "decorative", "wall art"],
    10.0, 10.0,
...
_design(
    ...
    "Classic Snowflake Ornament",
    "Six-arm symmetric snowflake, sized for a 4″ tree ornament. Single closed-path SVG.",
    "Holiday & Seasonal", "LASER", ["snowflake", "ornament", "christmas", "winter"],
    4.0, 4.0,
...
_design(
    ...
    "Six concentric topographic rings for stacked layered wall art. Single SVG with separable paths; perfect for modular 3D wood builds.",
    "Wall Art", "ROUTER", ["topographic", "contour", "mountain", "modular"],
    14.0, 14.0,
...
_design(
    ...
    "8-Point Compass Rose",
    "Maritime-style 8-point compass rose. Classic for cutting boards, leather, and outdoor signage. 8×8 square.",
    "Garden & Yard Art", "LASER", ["compass", "nautical", "maritime", "decorative"],
    8.0, 8.0,
...
_design(
    ...
    "Romantic heart silhouette wrapped in a leafy vine — for Mother's Day, anniversary, and wedding gift bases. 10×10.",
    "Wedding Gifts", "LASER", ["heart", "vine", "leaves", "romantic"],
    10.0, 10.0,
    '''<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1000 1000" width="1000" height="1000">
...
def _write_dxf(slug: str, segments) -> str:
    ...
    for points in segments:
        # ezdxf's LWPOLYLINE expects (x, y) pairs. Convert to a closed
        # polyline by appending the first point if not already closed.
        pts = list(points)
        if pts[0] != pts[-1]:
            pts.append(pts[0])
...
async def build_all():
    print(f"=== {len(SEED_DESIGNS)} seed designs ===")
    for d in SEED_DESIGNS:
        ...
        existing = await db.community_designs.find_one({"slug": slug}, {"_id": 0, "id": 1, "created_at": 1, "downloads": 1})
        if existing:
            doc["id"] = existing["id"]
            doc["created_at"] = existing["created_at"]
            doc["downloads"] = existing.get("downloads", 0)
        await db.community_designs.update_one({"slug": slug}, {"$set": doc}, upsert=True)
    print("\n=== done ===")
