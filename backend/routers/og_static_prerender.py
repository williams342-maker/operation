...
async def og_policy(http_request: Request):
    ...
    body_html = (
        ...
        '</section>'
        # iter329b — Google API Services User Data disclosure with the
        # EXACT section titles Google's OAuth verification reviewer
        # greps for (Google User Data We Access · How We Use Google Data
        # · Storage & Retention · Sharing & Third Parties · User Controls
        # / Disconnect & Delete Data · Google API Limited Use Disclosure).
        # Mirrors the React /policy page 1:1.
        ...
        '<h3 id="google-user-data-we-access">Google User Data We Access</h3>'
        '<ul>'
        '<li><strong>Google Search Console</strong> — scope: '
        '<code>https://www.googleapis.com/auth/webmasters</code>. '
        'We submit our own sitemaps and read indexing status of our own '
        'URLs (craftersmarket.org). We do not read data about other '
        'websites or users.</li>'
        '<li><strong>Google Analytics 4</strong> — scope: '
        '<code>https://www.googleapis.com/auth/analytics.readonly</code>. '
        'Server-side reporting on aggregate traffic to craftersmarket.org. '
        'We read aggregate metric counts only — no per-visitor identifiers '
        'are pulled into our database.</li>'
        '<li><strong>Google Ads API</strong> — scope: '
        '<code>https://www.googleapis.com/auth/adwords</code>. We create '
        '+ report on the off-site ad campaigns we run to drive traffic to '
        'craftersmarket.org. We do not access ad accounts that don\'t '
        'belong to us.</li>'
        '<li><strong>Connected-account email (display only)</strong> — '
        'immediately after the OAuth handshake, we make a best-effort '
        'call to Google\'s '
        '<code>https://www.googleapis.com/oauth2/v2/userinfo</code> '
        'endpoint to show "Connected as you@example.com" on the admin '
        'settings screen so the connecting team member can verify they '
        'linked the intended account. We do NOT request the '
        '<code>openid</code>, <code>profile</code>, or '
        '<code>userinfo.email</code> scopes separately — if Google\'s '
        'response does not include an email, we proceed with an empty '
        'value.</li>'
        '</ul>'

        '<h3 id="how-we-use-google-data">How We Use Google Data</h3>'
        '<ul>'
        '<li>Submit <code>sitemap.xml</code> + <code>sitemap_index.xml</code> '
        'to Google Search Console so Crafters Market pages are indexed.</li>'
        '<li>Inspect indexing status of specific craftersmarket.org URLs '
        'to debug indexing issues.</li>'
        '<li>Read aggregate Google Analytics 4 reports (pageviews, sources, '
        'sessions) for the admin dashboard.</li>'
        '<li>Create, pause, and read performance metrics on Google Ads '
        'campaigns we run to drive traffic to craftersmarket.org.</li>'
        '<li>Display the connected admin\'s email address on our admin '
        'settings screen.</li>'
        '<li>We DO NOT use Google user data to train AI or machine-learning '
        'models.</li>'
        '<li>We DO NOT use Google user data for advertising targeting '
        'outside our own owned Google Ads campaigns.</li>'
        '<li>We DO NOT enrich, profile, or repackage Google user data for '
        'any other purpose.</li>'
        '</ul>'

        '<h3 id="storage-and-retention">Storage &amp; Retention</h3>'
        '<ul>'
        '<li>An encrypted OAuth refresh token, stored at rest in our MongoDB.</li>'
        '<li>The connected admin\'s email address (display-only on the '
        'settings screen).</li>'
        '<li>The list of OAuth scopes granted.</li>'
        '<li>We DO NOT persist the content of Search Console reports, '
        'Analytics rows, or Ads campaigns beyond what\'s needed to render '
        'the current admin view in memory.</li>'
        '<li>Refresh tokens are deleted immediately on disconnect; any '
        'cached derived data is purged within 30 days.</li>'
        '<li>Data is stored in encrypted form on infrastructure provided '
        'by our hosting partner under industry-standard SOC 2 / ISO 27001 '
        'controls.</li>'
        '</ul>'

        '<h3 id="sharing-and-third-parties">Sharing &amp; Third Parties</h3>'
        '<ul>'
        '<li>We DO NOT sell Google user data.</li>'
        '<li>We DO NOT share Google user data with any third party for '
        'advertising, marketing, or analytics outside the Crafters Market '
        'platform.</li>'
        '<li>We DO NOT transfer Google user data to any AI or ML provider '
        'for training.</li>'
        '<li>We DO NOT share Google user data with sub-processors except '
        'the encrypted-at-rest database described in "Storage &amp; '
        'Retention" above.</li>'
        '<li>If we ever need to expand sharing for a legitimate '
        'operational reason, we will obtain explicit re-consent from the '
        'connecting admin before doing so.</li>'
        '</ul>'

        '<h3 id="user-controls-disconnect-and-delete">'
        'User Controls / Disconnect &amp; Delete Data</h3>'
        '<ul>'
        '<li>Revoke Google\'s grant at any time: visit '
        '<a href="https://myaccount.google.com/permissions">'
        'myaccount.google.com/permissions</a>, find "Crafters Market", '
        'and click "Remove access". Revocation is enforced by Google '
        'immediately.</li>'
        '<li>Disconnect from inside our admin dashboard: open '
        '<em>Admin → Settings → Integrations</em> and click "Disconnect" '
        'next to the relevant Google service.</li>'
        '<li>On disconnect, our stored refresh token is deleted within '
        'seconds. Any cached derived data (admin display state, sitemap '
        'submission history) is purged within 30 days.</li>'
        '<li>Request a copy of all Google-sourced data we hold about you: '
        'email <a href="mailto:team@craftersmarket.org">'
        'team@craftersmarket.org</a> with subject line "Google data access '
        'request". We respond within 30 days.</li>'
        '<li>Request deletion of all Google-sourced data we hold about '
        'you: email <a href="mailto:team@craftersmarket.org">'
        'team@craftersmarket.org</a> with subject line "Google data '
        'deletion request". We respond within 30 days.</li>'
        '</ul>'

        '<h3 id="google-api-limited-use-disclosure">'
        'Google API Limited Use Disclosure</h3>'
        '<p>Crafters Market\'s use and transfer to any other app of '
        ...
        'Google API Services User Data Policy</a>, including the Limited '
        'Use requirements.</p>'
        '<ul>'
        '<li>We affirm that data obtained from Google\'s APIs is used '
        'only to provide or improve user-facing features that are '
        'prominent in the requesting application\'s user experience — '
        'specifically the owner-side Search Console, Analytics, and Ads '
        'operations described above.</li>'
        '<li>We affirm that data obtained from Google\'s APIs is not '
        'transferred to others except as necessary to provide or improve '
        'those features, comply with applicable law, or as part of a '
        'merger, acquisition, or sale of assets with notice to users.</li>'
        '<li>We affirm that data obtained from Google\'s APIs is not '
        'used for serving advertisements outside our own owned Crafters '
        'Market ad campaigns.</li>'
        '<li>We affirm that humans do not read data obtained from '
        'Google\'s APIs unless we have obtained the connecting admin\'s '
        'affirmative agreement, the data is required for security '
        'purposes (such as investigating abuse), it is required to '
        'comply with applicable law, or the data has been aggregated and '
        'anonymized for internal operations.</li>'
        '</ul>'

        '<h3>Contact</h3>'
        '<p>Questions about how we handle Google user data: '
        '<a href="mailto:team@craftersmarket.org">team@craftersmarket.org</a>.</p>'
        '</section>'
