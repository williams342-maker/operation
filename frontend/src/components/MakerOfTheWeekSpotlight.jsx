...
export default function MakerOfTheWeekSpotlight({ testId = "maker-of-week" }) {
  ...
  if (data === null) {
    return (
      ...
      >
        <div className="border border-[#262626] bg-[#0d0d0d] p-6 md:p-8">
          <div className="grid md:grid-cols-[1fr_1.4fr] gap-8 items-start">
            <div className="space-y-3">
              ...
              <div className="h-9 w-40 bg-[#1a1a1a] animate-pulse mt-4" />
            </div>
  ...
  return (
    ...
    >
      ...
      <div className="border-l-2 border-[#ff4500] bg-gradient-to-r from-[#ff4500]/8 via-[#0d0d0d] to-[#0a0a0a] p-6 md:p-8">
        <div className="grid md:grid-cols-[1fr_1.4fr] gap-6 md:gap-10 items-start">
          ...
          <div className="min-w-0">
            ...
            >
              {maker.portrait ? (
                <img
                  ...
                  alt={maker.name}
                  loading="lazy"
                  className="w-14 h-14 object-cover border border-[#262626] group-hover:border-[#ff4500] transition"
                />
              ) : (
                <div className="w-14 h-14 grid place-items-center border border-[#262626] font-display text-xl text-[#a3a3a3] group-hover:border-[#ff4500] transition">
                  {maker.initials || maker.name?.[0] || "M"}
                </div>
              )}
              <div className="min-w-0">
                <h3 className="font-display text-2xl md:text-3xl text-[#e5e5e5] leading-tight group-hover:text-[#ff4500] transition truncate">
                  {maker.name}
                </h3>
                {maker.location && (
                  <p className="font-mono text-[10px] uppercase tracking-[0.22em] text-[#737373] mt-0.5">
                    {maker.location}
                  </p>
                )}
              </div>
            ...
            <div className="flex flex-wrap items-center gap-1.5 mt-3">
              ...
              {maker.subscription_status === "active" && (
                <span className="tag text-[#ff4500] border-[#ff4500] text-[9px]">
                  ◆ PLUS
                </span>
              )}
            ...
            {maker.bio && (
              <p className="font-mono text-xs text-[#a3a3a3] mt-4 leading-relaxed line-clamp-4">
                {maker.bio}
              </p>
            )}
            ...
            {isTrending && weeklyViews > 0 && (
              <p
                className="font-mono text-[11px] uppercase tracking-[0.22em] text-[#ff4500] mt-4 inline-flex items-center gap-1.5"
          ...
          <div className="grid grid-cols-3 gap-2 md:gap-3 min-w-0">
            {topPosts.length === 0 ? (
              <div className="col-span-3 font-mono text-xs text-[#737373] italic">
                ...
                return (
                  ...
                  >
                    {cover ? (
                      ...
                      <div className="w-full h-full grid place-items-center font-mono text-[9px] uppercase tracking-[0.22em] text-[#525252]">
                        No image
                      </div>
                    )}
                    {/* Lifetime view chip — always shown so the homepage
                        visitor sees concrete "popularity" evidence. */}
                    <span className="absolute bottom-1.5 left-1.5 bg-[#0a0a0a]/85 border border-[#262626] text-[#a3a3a3] font-mono text-[9px] px-1.5 py-0.5 flex items-center gap-1 pointer-events-none">
