    ...
    <section className="relative w-full py-24 md:py-32 bg-[#0a0a0a]">
      <div className="w-full max-w-[1800px] mx-auto px-4 md:px-8 xl:px-12">
        <div className="grid md:grid-cols-12 gap-10 mb-14">
          ...
          <div className="md:col-span-4 md:col-start-9 self-end font-mono text-sm text-[#a3a3a3] max-w-sm">
            <div className="text-4xl text-[#ff4500] font-display">4.97 / 5</div>
            <div className="mt-2">Average rating · 312 verified reviews · 98% repeat buyers.</div>
          </div>
        </div>

        <div className="grid md:grid-cols-3 gap-0 border-y border-[#262626]">
          {reviews.map((r, i) => (
            <motion.figure
              key={r.name}
              ...
              className={`relative p-8 md:p-12 ${
                i !== reviews.length - 1 ? "md:border-r border-[#262626]" : ""
              } border-b md:border-b-0 hover:bg-[#0f0f0f] transition-colors duration-500`}
              data-testid={`review-${i}`}
            >
              <Quote size={32} className="text-[#ff4500] mb-6" />
              <blockquote className="font-display text-2xl md:text-3xl leading-tight mb-10">
                {r.text}
              </blockquote>
              <figcaption className="flex items-center justify-between pt-6 border-t border-[#262626]">
                <div>
                  <div className="font-mono text-xs uppercase tracking-[0.2em] text-[#e5e5e5]">
                    {r.name}
                  </div>
                  <div className="font-mono text-[10px] uppercase tracking-[0.25em] text-[#a3a3a3] mt-1">
                    {r.loc}
                  </div>
                </div>
                <div className="font-mono text-[10px] tracking-[0.25em] text-[#ff4500]">
                  {"★".repeat(r.rating)}
                </div>
              </figcaption>
            </motion.figure>
