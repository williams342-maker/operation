...
export default function ProductRail({ title, eyebrow, category, technique, featured, viewAllHref = "/shop", testId }) {
  ...
  return (
    <section className="w-full py-14 md:py-16 bg-paper border-b border-line" data-testid={testId || "product-rail"}>
      <div className="w-full max-w-[1800px] mx-auto px-4 md:px-8 xl:px-12">
        <div className="flex items-end justify-between mb-8 gap-4">
          <div>
            {eyebrow && <div className="font-mono text-[11px] uppercase tracking-[0.3em] text-brand mb-2">{eyebrow}</div>}
            <h2 className="font-display text-3xl md:text-5xl lg:text-6xl">{title}</h2>
          </div>
          <div className="flex items-center gap-3">
            <button onClick={() => scroll(-1)} aria-label="Scroll left"
              className="hidden md:inline-flex w-10 h-10 border border-line hover:border-brand hover:text-brand items-center justify-center transition">
              <ChevronLeft size={18} />
            </button>
            ...
            <Link to={viewAllHref} className="industrial-link font-mono text-[11px] uppercase tracking-[0.22em] text-ink-muted hover:text-brand whitespace-nowrap">
              See all →
            </Link>
          </div>
        </div>
        ...
        <div ref={ref} className="flex gap-5 overflow-x-auto pb-4 snap-x snap-mandatory scrollbar-hide -mx-4 px-4 md:mx-0 md:px-0">
          {items.map((p, i) => (
            <motion.article
              ...
              className="snap-start flex-shrink-0 w-[260px] md:w-[300px] bg-surface border border-line hover:border-brand transition-colors duration-500"
            ...
            >
              <Link to={`/shop/${p.slug}`} className="block" onClick={() => trackPricingLabelClick(p.slug)}>
                ...
                <div className="p-4">
                  ...
                  <div className="font-mono text-[10px] uppercase tracking-[0.22em] text-ink-muted mb-3 line-clamp-1">
                    {p.dimensions || p.category}
                  </div>
                  <div className="flex items-end justify-between">
                    <div className="font-display text-2xl text-brand">{formatPriceDisplay(p)}</div>
                    <div className="font-mono text-[10px] uppercase tracking-[0.22em] text-ink-muted">+ ship</div>
                  </div>
                </div>
              </Link>
            </motion.article>
