...
export default function PromoStrip() {
  return (
    <section className="w-full py-12 md:py-16 bg-paper border-b border-line" data-testid="promo-strip">
      <div className="w-full max-w-[1800px] mx-auto px-4 md:px-8 xl:px-12 grid md:grid-cols-2 gap-6">
        {PROMOS.map((p, i) => (
          <Link key={p.title} to={p.href} data-testid={`promo-${i}`}
            className="group relative overflow-hidden bg-surface border border-line hover:border-brand transition p-8 md:p-12 min-h-[260px] flex flex-col justify-between">
            ...
            <div className="relative">
              ...
              <p className="font-mono text-xs text-ink-muted leading-relaxed mt-4 max-w-sm">{p.desc}</p>
            </div>
