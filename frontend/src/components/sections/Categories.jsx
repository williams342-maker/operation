...
export default function Categories() {
  return (
    <section id="categories" className="relative w-full py-24 md:py-32 bg-paper">
      <div className="w-full max-w-[1800px] mx-auto px-4 md:px-8 xl:px-12">
        ...
        <ul className="border-t border-line" data-testid="categories-list">
          {categories.map((c, i) => (
            <motion.li
              key={c.no}
              ...
              className="group relative border-b border-line hover:bg-paper transition-colors duration-500"
            ...
            >
              <Link to={`/shop?category=${encodeURIComponent(c.title)}`} className="grid grid-cols-12 gap-4 md:gap-8 py-10 md:py-14 items-center px-2 md:px-6">
                <div className="col-span-2 md:col-span-1 font-mono text-sm md:text-lg text-ink-muted group-hover:text-brand transition">
                  {c.no}
                </div>
                <div className="col-span-10 md:col-span-5">
                  ...
                  <div className="font-mono text-[11px] uppercase tracking-[0.3em] text-ink-muted mt-2 md:mt-4">
                    {c.sub}
                  </div>
                </div>
                <div className="hidden md:block md:col-span-3 font-mono text-sm text-ink-muted">
                  {c.blurb}
                </div>
                <div className="hidden md:flex md:col-span-2 items-center justify-end font-mono text-xs uppercase tracking-[0.25em] text-ink-muted">
                  {c.pieces} pieces
                </div>
                <div className="col-span-12 md:col-span-1 flex md:justify-end">
                  <span className="font-mono text-xs uppercase tracking-[0.2em] text-ink group-hover:text-brand transition">
                    Explore →
                  </span>
                </div>
                ...
                {/* Hover preview image */}
                <motion.div
                  className="pointer-events-none absolute right-6 md:right-14 top-1/2 -translate-y-1/2 w-40 h-52 md:w-64 md:h-80 overflow-hidden border border-brand"
