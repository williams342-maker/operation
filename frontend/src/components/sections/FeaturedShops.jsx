...
export default function FeaturedShops() {
  ...
  return (
    <section className="w-full py-16 md:py-20 bg-[#0a0a0a] border-b border-[#262626]" data-testid="featured-shops">
      <div className="w-full max-w-[1800px] mx-auto px-4 md:px-8 xl:px-12">
        <div className="flex items-end justify-between mb-10 gap-4">
          <div>
            <div className="font-mono text-[11px] uppercase tracking-[0.3em] text-[#ff4500] mb-2">◆ Featured Shops</div>
            <h2 className="font-display text-3xl md:text-5xl lg:text-6xl">Workshops to Watch</h2>
          </div>
          <Link to="/makers" className="industrial-link font-mono text-[11px] uppercase tracking-[0.22em] text-[#a3a3a3] hover:text-[#ff4500] whitespace-nowrap">All shops →</Link>
        </div>
                  ...
                  <div className="col-span-7 p-5 md:p-6 flex flex-col justify-between">
                    <div>
                      <div className="flex items-center gap-2 mb-2">
                        <div className="w-7 h-7 bg-[#1a1a1a] border border-[#ff4500] flex items-center justify-center font-mono text-[10px] text-[#ff4500]">
                          {m.initials}
                        </div>
                        <div className="font-mono text-[10px] uppercase tracking-[0.22em] text-[#a3a3a3]">{m.location}</div>
                      </div>
                      <div className="font-display text-2xl md:text-3xl mb-2 group-hover:text-[#ff4500] transition">{m.name}</div>
                      <p className="font-mono text-[11px] text-[#a3a3a3] leading-relaxed line-clamp-3">{m.bio}</p>
                    </div>
                    <div className="flex items-center justify-between mt-4 pt-3 border-t border-[#262626]">
                      ...
                      <div className="font-mono text-[10px] uppercase tracking-[0.22em] text-[#ff4500]">★ {m.rating}</div>
                    </div>
                  </div>
                </div>
              </Link>
