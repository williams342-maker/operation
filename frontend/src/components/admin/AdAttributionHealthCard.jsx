...
export default function AdAttributionHealthCard() {
  ...
  if (!data) {
    return (
      <div className="border border-line bg-paper p-5" data-testid="ad-attribution-health-card">
        <div className="flex items-center gap-2 text-ink-muted font-mono text-[10px] uppercase tracking-[0.22em]">
    ...
    <div className="border border-line bg-paper p-5" data-testid="ad-attribution-health-card">
      <header className="flex items-center justify-between flex-wrap gap-2 mb-4">
        <div>
          <div className="flex items-center gap-2 text-ink-muted font-mono text-[10px] uppercase tracking-[0.22em]">
            <ShieldCheck size={12} /> Ad attribution health
            <span className="text-ink-muted">· last {data.window_days}d</span>
          </div>
      ...
      <div className="grid sm:grid-cols-3 gap-3">
        <div className="border border-line p-3" data-testid="health-stat-coverage">
          <div className="font-mono text-[9px] uppercase tracking-[0.22em] text-ink-muted">Click-ID coverage</div>
          <div className={`font-display text-3xl mt-1 ${rateTone(coverage)}`}>
            {fmtPct(coverage)}
          </div>
          <div className="font-mono text-[10px] text-ink-muted mt-1 tabular-nums">
            {fmtInt(data.sessions_with_click_id)} / {fmtInt(data.paid_sessions)} paid
          </div>
        </div>
        <div className="border border-line p-3" data-testid="health-stat-paid">
          <div className="font-mono text-[9px] uppercase tracking-[0.22em] text-ink-muted">Paid sessions</div>
          <div className="font-display text-3xl mt-1 text-ink">{fmtInt(data.paid_sessions)}</div>
          <div className="font-mono text-[10px] text-ink-muted mt-1">last {data.window_days} days</div>
        </div>
        <div className={`border ${backlog > 0 ? "border-amber-900/50 bg-amber-950/20" : "border-line"} p-3`} data-testid="health-stat-backlog">
