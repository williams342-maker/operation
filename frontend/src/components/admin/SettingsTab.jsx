...
// iter251 — Social auto-posting card. Surfaces the new Buffer endpoints
// (auto-settings toggle · auto-pick-now manual trigger · social-history
// log). The clip/showcase share-buttons themselves live on the public
// /clips and /community pages — this card is the centralised control room.
function SocialAutoPostCard() {
  ...
  const load = async () => {
    setErr("");
    try {
      const s = await authed("/api/admin/buffer/auto-settings");
      if (!s.ok) throw new Error(`HTTP ${s.status}`);
      const sj = await s.json();
      setEnabled(!!sj.enabled);
      const h = await authed("/api/admin/buffer/social-history?limit=10");
      if (h.ok) setHistory((await h.json()).items || []);
    } catch (e) {
      setErr(e.message || "Failed to load");
    }
  };
  ...
  const toggle = async () => {
    ...
    setErr("");
    try {
      const r = await authed("/api/admin/buffer/auto-settings", {
        method: "POST",
  ...
  const pickNow = async () => {
    ...
    setPickResult(null);
    try {
      const r = await authed("/api/admin/buffer/auto-pick-now", { method: "POST" });
      if (!r.ok) throw new Error(`HTTP ${r.status}`);
      const j = await r.json();
      setPickResult(j);
  ...
  return (
    <section className="border border-[#262626] p-4 md:p-5" data-testid="social-autopost-card">
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div>
          <div className="font-mono text-[10px] uppercase tracking-[0.22em] text-[#a3a3a3]">
            Buffer · clips + showcase
          </div>
          <h3 className="font-display text-xl mt-1 text-[#e5e5e5]">Social auto-posts</h3>
          <p className="font-mono text-xs text-[#a3a3a3] mt-2 max-w-2xl leading-relaxed">
            Push the top-engaging clip + showcase post from the last 24h to every
            connected social channel (Instagram, Facebook, Threads, Pinterest, Twitter,
            LinkedIn, Mastodon, Bluesky). Captions auto-include the maker handle,
            a UTM-tagged link back to craftersmarket.org, and hashtags.
          </p>
        </div>
        <button
          type="button"
          onClick={load}
          className="shrink-0 px-3 py-1.5 border border-[#262626] hover:border-[#ff4500] hover:text-[#ff4500] font-mono text-[10px] uppercase tracking-[0.22em] transition"
          data-testid="social-autopost-refresh"
        >
          ↻ Refresh
        </button>
      </div>
      ...
      {err && (
        <div className="mt-3 font-mono text-xs text-red-400 break-all" data-testid="social-autopost-error">
          ✗ {err}
        </div>
      )}
      ...
      <div className="mt-5 grid md:grid-cols-2 gap-3">
        {/* Toggle */}
        <div className="p-3 border border-[#1a1a1a] bg-[#0a0a0a]">
          <div className="font-mono text-[10px] uppercase tracking-[0.22em] text-[#a3a3a3] mb-2">
            Nightly auto-pick · runs at 9am ET
          </div>
          <div className="flex items-center justify-between gap-3">
            <span className={`font-mono text-sm ${enabled ? "text-emerald-400" : "text-[#a3a3a3]"}`}>
            ...
            <button
              ...
              className={`px-3 py-1.5 font-mono text-[10px] uppercase tracking-[0.22em] font-bold disabled:opacity-40 ${
                enabled
                  ? "bg-[#262626] hover:bg-[#3a3a3a] text-[#e5e5e5]"
                  : "bg-[#ff4500] hover:bg-[#ff6a2a] text-black"
              }`}
              data-testid="social-autopost-toggle"
            >
        ...
        <div className="p-3 border border-[#1a1a1a] bg-[#0a0a0a]">
          ...
          <p className="font-mono text-[11px] text-[#a3a3a3] mb-3 leading-relaxed">
            Pick the top clip + showcase right now and queue both.
          </p>
          <button
            type="button"
            onClick={pickNow}
            disabled={picking || !enabled}
            className="w-full px-3 py-2 bg-[#ff4500] hover:bg-[#ff6a2a] disabled:opacity-40 text-black font-mono text-[10px] uppercase tracking-[0.22em] font-bold"
            data-testid="social-autopost-pick"
          >
            {picking ? "Queueing…" : "Pick & post now"}
          </button>
          {!enabled && <div className="font-mono text-[9px] text-[#525252] mt-1.5">Enable nightly first</div>}
        </div>
      </div>

      {pickResult && (
        <div
          className="mt-4 p-3 border border-emerald-500/60 bg-emerald-500/5 font-mono text-[11px] text-emerald-400"
          data-testid="social-autopost-result"
        >
          ✓ Queued. Clip: {pickResult.clip ? (pickResult.clip.slug || pickResult.clip.error) : "—"} ·
          Showcase: {pickResult.showcase ? (pickResult.showcase.title || pickResult.showcase.error) : "—"}
        </div>
      )}
      ...
      <div className="mt-5">
        ...
        {history.length === 0 ? (
          <div className="font-mono text-[11px] text-[#525252]">No social shares yet.</div>
        ) : (
          <ul className="space-y-1.5" data-testid="social-autopost-history">
            {history.map((h, i) => {
              const success = h.buffer_result?.success_count || 0;
              const fail = h.buffer_result?.fail_count || 0;
              const ref = h.source_slug || h.source_id || "—";
              return (
                <li key={i} className="font-mono text-[11px] text-[#a3a3a3] flex items-center gap-3 border border-[#1a1a1a] px-2.5 py-1.5">
                  <span className="text-[#e5e5e5]">{h.kind}</span>
                  <span className="text-[#525252]">·</span>
                  <span className="flex-1 truncate">{ref}</span>
                  <span className={success > 0 ? "text-emerald-400" : "text-[#525252]"}>{success}✓</span>
                  {fail > 0 && <span className="text-red-400">{fail}✗</span>}
                  <span className="text-[#525252] text-[10px]">{(h.by || "").split("@")[0]}</span>
                </li>
              );
            })}
          </ul>
