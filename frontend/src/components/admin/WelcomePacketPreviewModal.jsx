...
export default function WelcomePacketPreviewModal({
  ...
  return (
    <div
      className="fixed inset-0 z-[120] bg-paper/80 backdrop-blur-sm flex items-center justify-center p-4"
      data-testid="welcome-packet-preview-modal"
      onClick={onClose}
    >
      <div
        className="w-full max-w-[920px] max-h-[92vh] bg-paper border border-line flex flex-col"
      ...
      >
        ...
        <div className="flex items-start justify-between gap-4 px-6 py-4 border-b border-line">
          <div>
            ...
            <div className="font-mono text-xs text-ink-muted mt-1 break-words">
              Will send to: <span className="text-ink">{data?.recipient || "…"}</span>
            </div>
          </div>
          <button
            type="button"
        ...
        {/* Tabs */}
        <div className="flex border-b border-line" data-testid="welcome-preview-tabs">
          {[
        ...
        <div className="flex-1 overflow-hidden flex flex-col md:flex-row">
          ...
          <div className="md:w-[280px] md:shrink-0 border-r border-line p-5 space-y-4 overflow-y-auto">
            <div>
              <div className="font-mono text-[10px] uppercase tracking-[0.22em] text-ink-muted mb-1">
                Subject line
              </div>
              <div className="font-mono text-xs text-ink break-words leading-relaxed">
                {data?.subject || "…"}
              </div>
            </div>
            <div>
              ...
              <div className="font-mono text-xs text-ink break-words">
                {applicantName}
              </div>
            </div>
            <div>
              <label className="font-mono text-[10px] uppercase tracking-[0.22em] text-ink-muted mb-1 block">
              ...
              />
              <p className="font-mono text-[9px] uppercase tracking-[0.22em] text-ink-muted mt-1 leading-relaxed">
                Renders live · note appears as a quoted block at the end of the email.
              </p>
            </div>
            <div className="border-t border-line pt-4">
              <p className="font-mono text-[10px] uppercase tracking-[0.22em] text-ink-muted leading-relaxed">
          ...
          <div className="flex-1 bg-surface overflow-hidden flex flex-col min-h-[60vh]">
            <div className="px-4 py-2 border-b border-line font-mono text-[10px] uppercase tracking-[0.22em] text-ink-muted">
              Inbox preview
            </div>
            {loading && !data ? (
              <div className="flex-1 flex items-center justify-center font-mono text-xs text-ink-muted">
                Rendering…
              </div>
            ) : (
              <iframe
