  ...
  return (
    <div className="pt-32 pb-24 grain min-h-screen" data-testid="journal-page">
      <div className="w-full max-w-[1400px] mx-auto px-4 md:px-8">
        <div className="font-mono text-[11px] uppercase tracking-[0.3em] text-brand mb-4">◆ Journal</div>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {posts.map((p) => (
            <Link key={p.id} to={`/journal/${p.slug}`} data-testid={`post-${p.slug}`}
              ...
              <div className="p-6">
                <div className="font-mono text-[10px] uppercase tracking-[0.25em] text-ink-muted mb-3">{p.author} · {p.read_min} min read</div>
                <h3 className="font-display text-2xl mb-3 group-hover:text-brand transition">{p.title}</h3>
                <p className="font-mono text-xs text-ink-muted leading-relaxed">{p.excerpt}</p>
              </div>
            </Link>
          ))}
        </div>
...
export function JournalDetail() {
  ...
  if (!p) return <div className="pt-40 text-center font-mono text-sm text-ink-muted">Loading…</div>;
  return (
    <article className="pt-32 pb-24 grain min-h-screen" data-testid="journal-detail">
      <div className="w-full max-w-[900px] mx-auto px-4 md:px-8">
        <Link to="/journal" className="industrial-link font-mono text-[11px] uppercase tracking-[0.22em] text-ink-muted hover:text-brand mb-6 inline-block">← All entries</Link>
        <div className="flex justify-between items-start gap-4 flex-wrap mb-4">
          <div className="font-mono text-[10px] uppercase tracking-[0.25em] text-ink-muted">{p.author} · {p.read_min} min read</div>
        ...
        <div className="aspect-[16/9] mb-10 overflow-hidden border border-line">
