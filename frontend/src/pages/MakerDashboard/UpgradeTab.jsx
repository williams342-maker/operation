  ...
  return (
    <div className="space-y-8" data-testid="upgrade-tab">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        {/* Free tier */}
        <div className="border border-line bg-paper p-6" data-testid="upgrade-free">
          <div className="font-mono text-[10px] uppercase tracking-[0.22em] text-ink-muted mb-2">
            Current plan {!isPlus && <span className="text-brand">· you are here</span>}
          </div>
          <h3 className="font-display text-3xl uppercase mb-1">Free</h3>
          <div className="font-mono text-sm text-ink-muted mb-6">$0 / month</div>
          <ul className="space-y-2.5 font-mono text-xs text-ink">
            <li className="flex gap-2"><Check size={14} className="text-ink-muted shrink-0 mt-0.5" /> First {free.listings} listings free for life · $0.20 each after</li>
            <li className="flex gap-2"><Check size={14} className="text-ink-muted shrink-0 mt-0.5" /> {free.commission}% commission + {free.processing}% processing = <b className="text-brand">{free.total}% deducted per sale</b></li>
            <li className="flex gap-2"><Check size={14} className="text-ink-muted shrink-0 mt-0.5" /> Stripe Connect payouts</li>
            <li className="flex gap-2"><Check size={14} className="text-ink-muted shrink-0 mt-0.5" /> Standard placement in feeds</li>
          </ul>
        </div>

        {/* Plus tier */}
        <div className="border-2 border-brand bg-brand/5 p-6 relative" data-testid="upgrade-plus">
          <div className="absolute top-0 right-4 -translate-y-1/2 bg-brand text-[#0a0a0a] px-3 py-1 font-mono text-[10px] uppercase tracking-[0.22em]">
          ...
          <h3 className="font-display text-3xl uppercase mb-1 flex items-center gap-2">
            <Sparkles size={22} className="text-brand" /> Crafters Plus
          </h3>
          <div className="font-mono text-sm text-ink mb-2">
            ${plus.price} / month
            {trialEligible && !isPlus && (
              <span className="ml-2 text-brand">· {trialMonths}-month free trial</span>
            )}
          </div>
          {trialEligible && !isPlus && (
            <div
              className="font-mono text-[10px] text-ink-muted mb-4 leading-relaxed"
          ...
          <ul className="space-y-2.5 font-mono text-xs text-ink">
            <li className="flex gap-2"><Check size={14} className="text-brand shrink-0 mt-0.5" /> {plus.listings} new listings every month, free</li>
            <li className="flex gap-2"><Check size={14} className="text-brand shrink-0 mt-0.5" /> {plus.commission}% commission (vs {free.commission}%) + {free.processing}% processing = <b className="text-brand">{plus.total}% deducted per sale</b></li>
            <li className="flex gap-2"><Check size={14} className="text-brand shrink-0 mt-0.5" /> Custom shop banner image</li>
            <li className="flex gap-2"><Check size={14} className="text-brand shrink-0 mt-0.5" /> Priority placement in homepage rotations</li>
            <li className="flex gap-2"><Check size={14} className="text-brand shrink-0 mt-0.5" /> Plus-only "Maker Spotlight" features</li>
            <li className="flex gap-2"><Check size={14} className="text-brand shrink-0 mt-0.5" /> Cancel anytime — takes effect end of cycle</li>
          </ul>
      ...
      <div className="border border-line bg-paper p-5 text-center">
        <p className="font-mono text-[11px] text-ink-muted leading-relaxed">
          Plus pays for itself with about <b className="text-brand">$1,200/mo</b> in sales —
