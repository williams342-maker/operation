...
export default function CustomUrlPicker() {
  ...
  return (
    <section className="border border-[#262626] p-5" data-testid="custom-url-section">
      <div className="flex items-start gap-3 mb-4">
        <Sparkles size={16} className="text-[#ff4500] mt-1 shrink-0" />
        <div>
          ...
          <p className="font-mono text-xs text-[#a3a3a3] mt-2 leading-relaxed max-w-md">
            Claim a memorable URL like <span className="text-[#e5e5e5]">craftersmarket.org/makers/iron-and-oak</span>.
      ...
      {!editing && state.custom_url && (
        <div className="bg-[#0d0d0d] border border-[#1f1f1f] p-4 flex items-center gap-3 flex-wrap">
          <div className="flex-1 min-w-[200px] font-mono text-xs text-[#e5e5e5] break-all" data-testid="custom-url-current">
            <span className="text-[#737373]">{origin}/makers/</span>
            <span className="text-[#ff4500]">{state.custom_url}</span>
          </div>
          <button
            onClick={copy}
            className="px-3 py-1.5 border border-[#262626] hover:border-[#525252] font-mono text-[10px] uppercase tracking-[0.22em] text-[#a3a3a3] hover:text-[#e5e5e5] transition inline-flex items-center gap-1.5"
      ...
      {editing && (
        <div className="space-y-3">
          <label className="block font-mono text-[10px] uppercase tracking-[0.22em] text-[#a3a3a3]">
            Your vanity URL
          </label>
          <div className="flex items-stretch gap-2 flex-wrap">
            <span className="px-3 py-2 bg-[#0d0d0d] border border-[#262626] font-mono text-xs text-[#737373] flex items-center">
              {origin}/makers/
            </span>
            <input
              type="text"
              ...
              maxLength={30}
              className="flex-1 min-w-[160px] px-3 py-2 bg-[#0d0d0d] border border-[#262626] focus:border-[#ff4500] focus:outline-none font-mono text-sm text-[#e5e5e5]"
          ...
          <div className="font-mono text-[11px] min-h-[18px]" data-testid="custom-url-availability">
            {checking && <span className="text-[#737373]">Checking…</span>}
          ...
          <div className="font-mono text-[10px] text-[#525252] leading-relaxed">
            {state.rules}
          </div>
          <div className="flex gap-2 pt-2">
            ...
            <button
              onClick={cancelEdit}
              disabled={saving}
              className="px-3 py-2 border border-[#262626] hover:border-[#525252] font-mono text-[10px] uppercase tracking-[0.22em] text-[#a3a3a3] hover:text-[#e5e5e5] transition disabled:opacity-50"
...
function LockedCard() {
  return (
    <section
      className="border-2 border-dashed border-[#ff4500]/40 bg-[#ff4500]/5 p-5"
      data-testid="custom-url-locked"
    >
      <div className="inline-flex items-center gap-2 font-mono text-[10px] uppercase tracking-[0.22em] text-[#ff4500] mb-2">
        <Lock size={12} /> Plus subscribers only
      </div>
      <h3 className="font-display text-xl uppercase mb-2">Custom shop URL.</h3>
      <p className="font-mono text-xs text-[#a3a3a3] mb-4 max-w-md leading-relaxed">
        Replace your auto-generated shop ID with a memorable vanity URL —
        e.g. <span className="text-[#e5e5e5]">/makers/iron-and-oak</span>.
