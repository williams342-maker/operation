    ...
    return (
      <div className="grid lg:grid-cols-[1fr_1.4fr] gap-4" data-testid="renewal-summary-loading">
        <div className="border border-line bg-paper p-5 animate-pulse">
          <div className="h-3 w-24 bg-surface mb-2" />
          <div className="h-6 w-32 bg-surface mb-4" />
          <div className="grid grid-cols-3 gap-3">
            {[0, 1, 2].map((i) => (
              <div key={i} className="border border-line bg-paper p-3">
                <div className="h-7 w-8 mx-auto bg-surface mb-2" />
                <div className="h-2 w-12 mx-auto bg-surface" />
              </div>
            ))}
          </div>
        </div>
        <div className="border border-line bg-paper p-5 animate-pulse">
          <div className="h-3 w-24 bg-surface mb-4" />
  ...
  if (err) {
    return (
      ...
      >
        ...
        <div className="font-mono text-xs text-ink">{err}</div>
      </div>
    );
  }
  if (!data) return null;
...
function SummaryCard({ counts }) {
  return (
    <div className="border border-line bg-paper p-5" data-testid="renewal-summary-card">
      <div className="flex items-center justify-between mb-4">
        <div>
          <div className="font-mono text-[10px] uppercase tracking-[0.3em] text-brand">◆ Renewals</div>
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          { label: "Next 7d", n: counts.next_7d, tone: counts.next_7d > 0 ? "text-brand" : "text-ink-muted" },
          { label: "Next 14d", n: counts.next_14d, tone: "text-ink" },
          { label: "Next 30d", n: counts.next_30d, tone: "text-ink" },
        ].map((c) => (
          <div key={c.label} className="border border-line bg-paper p-3 text-center">
      ...
      <div className="mt-4 flex items-center justify-between gap-2 text-[10px] font-mono uppercase tracking-[0.22em]">
        <span className="text-ink-muted">
          <span className="text-emerald-700">●</span> {counts.total_auto} auto-renew
          <span className="mx-2 text-ink-muted/40">·</span>
          <span className="text-brand">●</span> {counts.total_manual} manual
        </span>
        <button
          type="button"
          onClick={() => {
...
function CalendarWidget({ calendar }) {
  ...
  return (
    <div className="border border-line bg-paper p-5" data-testid="renewal-calendar">
      <div className="flex items-center gap-2 mb-4">
        <CalIcon size={14} className="text-brand" />
        <div className="font-mono text-[10px] uppercase tracking-[0.3em] text-brand">◆ Calendar</div>
        <div className="ml-auto font-mono text-[10px] text-ink-muted">next 30 days · click a day</div>
      </div>
      <div className="grid grid-cols-7 gap-1.5">
        {weeks.flat().map((d) => {
          ...
          return (
            <button
              ...
              className={`aspect-square border flex flex-col items-center justify-center transition ${
                d.count === 0
                  ...
                  : "border-line hover:border-brand cursor-pointer"
              }`}
              style={{
            ...
            >
              <span className={`font-display text-base leading-none ${d.count === 0 ? "text-ink-muted" : "text-ink"}`}>
                {day}
              </span>
              {d.count > 0 && (
                <span className="font-mono text-[9px] mt-0.5 text-brand">{d.count}</span>
              )}
            </button>
          );
        })}
      ...
      {openDay && (
        <div
          className="mt-4 border border-brand/40 bg-brand/5 p-3"
        ...
        >
          ...
          <ul className="space-y-1">
            {(calendar.find((d) => d.date === openDay)?.listings || []).map((l) => (
              <li key={l.slug} className="font-mono text-xs text-ink">
                <Link to={`/maker/listings/${l.slug}/edit`} className="hover:text-brand">
                  → {l.title}
                </Link>
              </li>
            ))}
