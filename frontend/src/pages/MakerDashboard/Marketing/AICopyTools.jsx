    ...
    <Section title="AI Listing Copy" testId="ai-listing-copy">
      ...
      <form onSubmit={submit} className="space-y-3">
        <textarea value={bullets} onChange={(e) => setBullets(e.target.value)}
          ...
          className="w-full bg-paper border border-line focus:border-brand px-4 py-3 font-mono text-xs outline-none resize-y"
          data-testid="ai-copy-bullets" />
        <div className="grid grid-cols-2 gap-3">
          <input value={category} onChange={(e) => setCategory(e.target.value)} placeholder="Category"
            className="bg-paper border border-line focus:border-brand px-3 py-2 font-mono text-xs outline-none"
      ...
      {out && (
        <div className="mt-6 space-y-4 border-t border-line pt-5" data-testid="ai-copy-output">
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              <div className="font-mono text-[10px] uppercase tracking-[0.22em] text-ink-muted">Tags ({out.tags.length})</div>
              <button onClick={() => copy(out.tags.join(", "), "Tags")} className="font-mono text-[10px] uppercase tracking-[0.22em] text-brand hover:underline">
                <Copy size={11} className="inline" /> Copy all
              </button>
            </div>
            <div className="flex flex-wrap gap-1.5">
              {out.tags.map((t, i) => (
                <span key={i} className="px-2 py-1 border border-line bg-paper font-mono text-[11px] text-ink">{t}</span>
              ))}
            </div>
          </div>
        </div>
  ...
  return (
    <div data-testid={testid}>
      <div className="flex items-center justify-between mb-1.5">
        ...
        <button onClick={onCopy} className="font-mono text-[10px] uppercase tracking-[0.22em] text-brand hover:underline">
          <Copy size={11} className="inline" /> Copy
        </button>
      </div>
      <div className={`bg-paper border border-line p-3 font-mono text-xs text-ink leading-relaxed ${multiline ? "whitespace-pre-wrap" : "truncate"}`}>
        {value}
      </div>
    </div>
  );
...
export function SeoRecommender() {
  ...
  return (
    <Section title="SEO Recommender" testId="ai-seo-audit">
      ...
      {state.data && (
        <div className="space-y-4 border-t border-line pt-5 mt-2" data-testid="ai-seo-output">
          <p className="font-mono text-xs text-ink leading-relaxed">{state.data.summary}</p>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {state.data.missing_keywords.map((k, i) => (
                  <span key={i} className="px-2 py-1 border border-brand/40 bg-brand/5 font-mono text-[11px] text-brand">{k}</span>
                ))}
              </div>
            </div>
          )}
          {!!state.data.title_rewrites?.length && (
            <div>
              <div className="font-mono text-[10px] uppercase tracking-[0.22em] text-ink-muted mb-2">Title Rewrites</div>
              <div className="space-y-2">
                {state.data.title_rewrites.map((r, i) => (
                  <div key={i} className="border border-line p-3" data-testid={`seo-rewrite-${i}`}>
                    <div className="font-mono text-[10px] text-ink-muted line-through truncate">{r.current}</div>
                    <div className="font-mono text-xs text-ink mt-1">{r.suggested}</div>
                    <div className="font-mono text-[10px] text-ink-muted mt-1.5 italic">→ {r.reason}</div>
                  </div>
                ))}
              </div>
            </div>
    ...
    <Section title="Bulk SEO Tag Generator" testId="ai-seo-bulk">
      ...
      <div className="grid sm:grid-cols-2 gap-3 mb-4">
        <label className="block">
          <span className="font-mono text-[10px] uppercase tracking-[0.22em] text-ink-muted block mb-1.5">Tag threshold</span>
        ...
        <label className="block">
          <span className="font-mono text-[10px] uppercase tracking-[0.22em] text-ink-muted block mb-1.5">Max listings per run</span>
      ...
      {result && (
        <div className="mt-5 border border-line bg-paper" data-testid="seo-bulk-result">
          <div className="px-4 py-3 border-b border-line font-mono text-[10px] uppercase tracking-[0.22em] text-ink-muted">
            ◆ {result.scanned} listings scanned · <span className="text-brand">{result.total_added}</span> new tags added
          </div>
          {result.results.length === 0 ? (
            <div className="px-4 py-6 font-mono text-[11px] text-ink-muted text-center">No listings matched the threshold.</div>
          ) : (
            <div className="max-h-80 overflow-y-auto">
              {result.results.map((r) => (
                <div key={r.slug} className="px-4 py-3 border-b border-line flex items-start justify-between gap-3" data-testid={`seo-bulk-row-${r.slug}`}>
                  <div className="min-w-0 flex-1">
                    <div className="font-mono text-[12px] text-ink truncate">{r.title}</div>
                    {r.added_tags.length > 0 && (
                      <div className="flex flex-wrap gap-1 mt-1">
                        {r.added_tags.map((t) => (
                          <span key={t} className="font-mono text-[10px] px-1.5 py-0.5 border border-brand/40 text-brand bg-brand/5">{t}</span>
                        ))}
                      </div>
                    )}
                  </div>
                  <div className="font-mono text-[10px] text-ink-muted uppercase tracking-[0.18em] shrink-0 text-right">
                    +{r.added_count}
                    <div className="text-[9px] text-ink-muted normal-case mt-0.5">{r.total_tags_after}/13 total</div>
                  </div>
                </div>
              ))}
            </div>
